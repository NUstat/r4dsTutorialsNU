...
<!-- Modification of r4ds.tutorials authors: Anish Bellamkonda and David Kane-->
...
Compared to just passing in `y` in `unnest_wider()`, we can see the column name plus with the element name and that is how `unnest_wider()` works, now we will move into `unnest_longer()` which is more useful for data with different amounts of data for records.
...
But what happens if one of the elements is empty? To see what happens, let's first create another sample of data, so create a new tibble using `tribble()` and assign it to `df6`. 

Within `tribble()`, we will define the columns `~x` and `~y`. 
...
Add `df6` on a newline, so it prints.
...
Modify `select()` to select `id` and columns that are lists using `where(is.list)`.
...
Start a new pipe with `locations` to `select()` and we will select the following: `city`, `formatted_address`, and `geometry`. Then we will extend the pipe to `unnest_wider()` and pass in `geometry` to extend the list.
...
<!-- Somehow add the theoretical stuff about json -->
...
Create a new data object called `gh_user2` by assigning it the result of the `read_json()` function with `gh_users_json()` function as its argument.
...
Now, let's transform the JSON to a list and store it within a tibble. Create a variable called `df3` by using `tibble()`. Within the call to `tibble()`, we will have a json column where `parse_json(json)` is set equal to `json`. On a new line type `df3`, so the tibble prints out.
