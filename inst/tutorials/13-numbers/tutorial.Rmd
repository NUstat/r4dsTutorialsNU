...
Let's utilize a premade dataframe `df`. Type in `df` and hit "Submit Code".
...
On the next line compute the remainder of the vector `z` divided by `3` by typing in `z %% 3` and hitting "Submit Code". 
...
Let's round down to the nearest two digits. On the next line, type in `floor(x / 0.01) * 0.01` and hit "Submit Code".
...
Let's round up to the nearest two digits. On the next line, type in `ceiling(x / 0.01) * 0.01` and hit "Submit Code". 
...
Let's round to the nearest multiple of 4. On the next line, type in `round(x / 4) * 4` and hit "Submit Code".
...
This time let's round to the nearest 0.25. On the next line, type in `round(x / 0.25) * 0.25` and hit "Submit Code".
...
Type in `x == lag(x)` and hit "Submit Code".
...
Type in `times_visited` and hit "Submit Code".
...
Imagine you have a dataframe with a bunch of repeated values. Type in `repetition` and hit "Submit Code".
...
```{r general-transformations-26-solution, exercise.reveal_solution = FALSE}
repetition |> 
  summarize(id = consecutive_id(x), x, y) |>
  slice_head(n=1, by = id)
```

```{r general-transformations-26-check}
grade_this_code()
```
