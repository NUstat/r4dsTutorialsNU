...
<!-- Modification of r4ds.tutorials author: Srihith Garlapati -->
...
```{r making-numbers-1-solution, exercise.reveal_solution = FALSE}
library(tidyverse)
```

```{r making-numbers-1-check}
grade_this_code()
```
...
```{r making-numbers-2-solution, exercise.reveal_solution = FALSE}
x <- c("1.2", "5.6", "1e3")
```

```{r making-numbers-2-check}
grade_this_code()
```
### 
...
```{r making-numbers-3-solution, exercise.reveal_solution = FALSE}
x <- c("1.2", "5.6", "1e3")
parse_double(x)
```

```{r making-numbers-3-check}
grade_this_code()
```
...
```{r making-numbers-4-solution, exercise.reveal_solution = FALSE}
y <- c("$1,234", "USD 3,513", "59%")
```

```{r making-numbers-4-check}
grade_this_code()
```
...
```{r making-numbers-5-solution, exercise.reveal_solution = FALSE}
y <- c("$1,234", "USD 3,513", "59%")
parse_number(y)
```

```{r making-numbers-5-check}
grade_this_code()
```
...
Run `help(package = "dplyr")` at the Console. Find the help page for `count()`. Click on it. 
...
Type in `flights` and hit "Submit Answer".
...
```{r counts-1-solution, exercise.reveal_solution = FALSE}
flights
```
...
```{r counts-2-solution, exercise.reveal_solution = FALSE}
flights |> count(dest)
```
...
```{r counts-3-solution, exercise.reveal_solution = FALSE}
...
```{r counts-4-solution, exercise.reveal_solution = FALSE}
flights |> 
...
```{r counts-5-solution, exercise.reveal_solution = FALSE}
flights |> 
...
```{r counts-6-solution, exercise.reveal_solution = FALSE}
flights |> 
  summarize(carriers = n_distinct(carrier), .by = dest) |> 
  arrange(carriers)
```
...
```{r counts-7-solution, exercise.reveal_solution = FALSE}
flights |> 
  summarize(carriers = n_distinct(carrier), .by = dest) |> 
...
```{r counts-8-solution, exercise.reveal_solution = FALSE}
flights |> 
  count(tailnum)
```
...
```{r counts-9-solution, exercise.reveal_solution = FALSE}
flights |> 
...
Now, count the number of cancelled flights or missing values of each destination. 

With a new pipeline, pipe `flights` to the `summarize()` function. Within the call to `summarize()`, create a new variable called `n_cancelled` and set it equal to `sum(is.na(dep_time)`.
...
```{r counts-10-solution, exercise.reveal_solution = FALSE}
flights |> 
...
```{r counts-11-solution, exercise.reveal_solution = FALSE}
flights |> 
...
plots <- flights |> 
  ...
  labs(title = "Proportion of cancelled flights throughout the day",
       subtitle = "The most cancelled flights occur in the evening", 
       x = "Hour",
...
```{r numeric-transformations-1-solution, exercise.reveal_solution = FALSE}
x <- c(1, 2, 10, 20)
```

```{r numeric-transformations-1-check}
grade_this_code()
```

### Exercise 2

On the next line, divide the vector `x` by 5.
...
```{r numeric-transformations-2-solution, exercise.reveal_solution = FALSE}
x <- c(1, 2, 10, 20)
x / 5
```

```{r numeric-transformations-2-check}
grade_this_code()
```
...
On the next line, multiply the vector `x` with the values `c(1, 2, 3)`
...
```{r numeric-transformations-3-solution, exercise.reveal_solution = FALSE}
x <- c(1, 2, 10, 20)
x * c(1,2,3)
```

```{r numeric-transformations-3-check}
grade_this_code()
```
...
```{r numeric-transformations-4-solution, exercise.reveal_solution = FALSE}
flights |> 
  filter(month == c(1, 2))
```

```{r numeric-transformations-4-check}
grade_this_code()
```
...
Let's utilize a premade dataframe `df`. Type in `df` and hit "Submit Answer".
...
```{r numeric-transformations-5-solution, exercise.reveal_solution = FALSE}
df
```

```{r numeric-transformations-5-check}
grade_this_code()
```
...
```{r numeric-transformations-6-solution, exercise.reveal_solution = FALSE}
df |> 
  mutate(
    min = pmin(x, y)
  )
...
```{r numeric-transformations-6-check}
grade_this_code()
```
...
```{r numeric-transformations-7-solution, exercise.reveal_solution = FALSE}
df |> 
  mutate(
...
```{r numeric-transformations-7-check}
grade_this_code()
```
### 
...
```{r numeric-transformations-8-solution, exercise.reveal_solution = FALSE}
df |> 
  mutate(
    min = pmin(x, y, na.rm = TRUE), 
    max = pmax(x, y)
...
```{r numeric-transformations-8-check}
grade_this_code()
```
...
```{r numeric-transformations-9-solution, exercise.reveal_solution = FALSE}
df |> 
  mutate(
    min = pmin(x, y, na.rm = TRUE), 
...
```{r numeric-transformations-9-check}
grade_this_code()
```
...
```{r numeric-transformations-10-solution, exercise.reveal_solution = FALSE}
z <- 1:10
```

```{r numeric-transformations-10-check}
grade_this_code()
```

### Exercise 11

On the next line, divide `z` by 3 using integer division (`%/%`).
...
```{r numeric-transformations-11-solution, exercise.reveal_solution = FALSE}
z <- 1:10
z %/% 3
```

```{r numeric-transformations-11-check}
grade_this_code()
```
...
On the next line compute the remainder of the vector `z` divided by `3` by typing in `z %% 3` and hitting "Submit Answer". 
...
```{r numeric-transformations-12-solution, exercise.reveal_solution = FALSE}
z <- 1:10
z %% 3
```

```{r numeric-transformations-12-check}
grade_this_code()
```
...
```{r numeric-transformations-13-solution, exercise.reveal_solution = FALSE}
flights |> 
  mutate(
...
```{r numeric-transformations-13-check}
grade_this_code()
```
...
```{r numeric-transformations-14-solution, exercise.reveal_solution = FALSE}
flights |> 
  mutate(
    hour = sched_dep_time %/% 100, 
...
```{r numeric-transformations-14-check}
grade_this_code()
```
...
```{r numeric-transformations-15-solution, exercise.reveal_solution = FALSE}
flights |> 
  mutate(
    hour = sched_dep_time %/% 100, 
    minute = sched_dep_time %% 100, 
...
```{r numeric-transformations-15-check}
grade_this_code()
```
...
```{r numeric-transformations-16-solution, exercise.reveal_solution = FALSE}
flights |> 
  mutate(hour = sched_dep_time %/% 100)
```

```{r numeric-transformations-16-check}
grade_this_code()
```
...
Using your previous code. Pipe to `summarize()` and with in it create a new variable called `prop_cancelled` and set it to `mean(is.na(dep_time))`.
...
```{r numeric-transformations-17-solution, exercise.reveal_solution = FALSE}
flights |> 
  mutate(hour = sched_dep_time %/% 100) |> 
  summarize(
...
```{r numeric-transformations-17-check}
grade_this_code()
```
...
```{r numeric-transformations-18-solution, exercise.reveal_solution = FALSE}
flights |> 
  mutate(hour = sched_dep_time %/% 100) |> 
  summarize(
    prop_cancelled = mean(is.na(dep_time)),
...
```{r numeric-transformations-18-check}
grade_this_code()
```
...
```{r numeric-transformations-19-solution, exercise.reveal_solution = FALSE}
flights |>
  mutate(hour = sched_dep_time %/% 100) |> 
  summarize(
    prop_cancelled = mean(is.na(dep_time)),
...
```{r numeric-transformations-19-check}
grade_this_code()
```
...
```{r numeric-transformations-20-solution, exercise.reveal_solution = FALSE}
flights |>
  mutate(hour = sched_dep_time %/% 100) |> 
  summarize(
    prop_cancelled = mean(is.na(dep_time)),
...
```{r numeric-transformations-20-check}
grade_this_code()
```
...
```{r numeric-transformations-21-solution, exercise.reveal_solution = FALSE}
flights |>
  mutate(hour = sched_dep_time %/% 100) |> 
  summarize(
    prop_cancelled = mean(is.na(dep_time)),
...
```{r numeric-transformations-21-check}
grade_this_code()
```
...
```{r numeric-transformations-22-solution, exercise.reveal_solution = FALSE}
flights |>
  mutate(hour = sched_dep_time %/% 100) |> 
  summarize(
    prop_cancelled = mean(is.na(dep_time)),
...
```{r numeric-transformations-22-check}
grade_this_code()
```
...
```{r numeric-transformations-23-solution, exercise.reveal_solution = FALSE}
flights |>
  mutate(hour = sched_dep_time %/% 100) |> 
  summarize(
    prop_cancelled = mean(is.na(dep_time)),
...
```{r numeric-transformations-23-check}
grade_this_code()
```
...
```{r numeric-transformations-24-solution, exercise.reveal_solution = FALSE}
flights |>
  mutate(hour = sched_dep_time %/% 100) |> 
  summarize(
    prop_cancelled = mean(is.na(dep_time)),
...
```{r numeric-transformations-24-check}
grade_this_code()
```
...
Copy your previous code. Within the function `geom_point()` map the aesthetic `size` to `n`. 
...
```{r numeric-transformations-25-solution, exercise.reveal_solution = FALSE}
flights |>
  mutate(hour = sched_dep_time %/% 100) |> 
  summarize(
    prop_cancelled = mean(is.na(dep_time)),
...
```{r numeric-transformations-25-check}
grade_this_code()
```

### Exercise 26

Using your previous code, use the `labs()` function to set the title to "Proportion of cancelled flights throughout the day", subtitle to "The most cancelled flights occur in the evening", x axis label to  "Hour", and y axis label to "Proportion cancelled"
...
```{r numeric-transformations-26-solution, exercise.reveal_solution = FALSE}
flights |>
  mutate(hour = sched_dep_time %/% 100) |> 
  summarize(
    prop_cancelled = mean(is.na(dep_time)),
  ...
  geom_point(aes(size = n)) + 
  labs(
    title = "Proportion of cancelled flights throughout the day",
    subtitle = "The most cancelled flights occur in the evening", 
    x = "Hour",
    y = "Proportion cancelled"
  )
...
```{r numeric-transformations-26-check}
grade_this_code()
```
...
Use the function `round()` with the argument `123.456` and hit "Submit Answer".
...
```{r other-number-functions-2-solution, exercise.reveal_solution = FALSE}
round(123.456)
```

```{r other-number-functions-2-check}
grade_this_code()
```
...
```{r other-number-functions-3-solution, exercise.reveal_solution = FALSE}
round(123.456, digits = 2)
```

```{r other-number-functions-3-check}
grade_this_code()
```
...
```{r other-number-functions-4-solution, exercise.reveal_solution = FALSE}
round(123.456, digits = -2)
```

```{r other-number-functions-4-check}
grade_this_code()
```
...
Type in `round(c(1.5, 2.5))` and hit "Submit Answer".
...
```{r other-number-functions-5-solution, exercise.reveal_solution = FALSE}
round(c(1.5, 2.5))
```

```{r other-number-functions-5-check}
grade_this_code()
```
...
```{r other-number-functions-6-solution, exercise.reveal_solution = FALSE}
floor(123.456)
```

```{r other-number-functions-6-check}
grade_this_code()
```
...
```{r other-number-functions-7-solution, exercise.reveal_solution = FALSE}
ceiling(123.456)
```

```{r other-number-functions-7-check}
grade_this_code()
```
...
```{r other-number-functions-8-solution, exercise.reveal_solution = FALSE}
x <- 123.456
```

```{r other-number-functions-8-check}
grade_this_code()
```
...
Let's round down to the nearest two digits. On the next line, type in `floor(x / 0.01) * 0.01` and hit "Submit Answer".
...
```{r other-number-functions-9-solution, exercise.reveal_solution = FALSE}
x <- 123.456
floor(x / 0.01) * 0.01
```

```{r other-number-functions-9-check}
grade_this_code()
```

### Exercise 10

Let's round up to the nearest two digits. On the next line, type in `ceiling(x / 0.01) * 0.01` and hit "Submit Answer". 
...
```{r other-number-functions-10-solution, exercise.reveal_solution = FALSE}
x <- 123.456
ceiling(x / 0.01) * 0.01
```

```{r other-number-functions-10-check}
grade_this_code()
```
...
Let's round to the nearest multiple of 4. On the next line, type in `round(x / 4) * 4` and hit "Submit Answer".
...
```{r other-number-functions-11-solution, exercise.reveal_solution = FALSE}
x <- 123.456
round(x/4) * 4
```

```{r other-number-functions-11-check}
grade_this_code()
```

### Exercise 12

This time let's round to the nearest 0.25. On the next line, type in `round(x / 0.25) * 0.25` and hit "Submit Answer".
...
```{r other-number-functions-12-solution, exercise.reveal_solution = FALSE}
x <- 123.456
round(x / 0.25) * 0.25
```

```{r other-number-functions-12-check}
grade_this_code()
```
...
```{r other-number-functions-13-solution, exercise.reveal_solution = FALSE}
x <- c(1, 2, 5, 10, 15, 20)
```

```{r other-number-functions-13-check}
grade_this_code()
```
...
Copy the previous code and on the next line, use `cut()` on the numeric vector `x` and add in the `breaks` argument and set `breaks` to `c(0, 5, 10, 15, 20)`
...
```{r other-number-functions-14-solution, exercise.reveal_solution = FALSE}
x <- c(1, 2, 5, 10, 15, 20)
cut(x, breaks = c(0,5,10,15,20))
```

```{r other-number-functions-14-check}
grade_this_code()
```
...
```{r other-number-functions-15-solution, exercise.reveal_solution = FALSE}
x <- c(1, 2, 5, 10, 15, 20)
cut(x, breaks = c(0, 5, 10, 100))
```

```{r other-number-functions-15-check}
grade_this_code()
```
...
```{r other-number-functions-16-solution, exercise.reveal_solution = FALSE}
x <- c(1, 2, 5, 10, 15, 20)
cut(x, breaks = c(0,5,10,100), labels = c("sm", "md", "lg"))
```

```{r other-number-functions-16-check}
grade_this_code()
```
...
```{r other-number-functions-17-solution, exercise.reveal_solution = FALSE}
x <- 1:10
```

```{r other-number-functions-17-check}
grade_this_code()
```
...
On the next line, use the function `cumsum()` on `x`.
...
```{r other-number-functions-18-solution, exercise.reveal_solution = FALSE}
x <- 1:10
cumsum(x)
```

```{r other-number-functions-18-check}
grade_this_code()
```
...
Make a new vector named `x` that has the values of `c(1, 2, 2, 3, 4, NA)`.
...
```{r general-transformations-1-solution, exercise.reveal_solution = FALSE}
x <- c(1, 2, 2, 3, 4, NA)
```

```{r general-transformations-1-check}
grade_this_code()
```
...
On the next line, use the function `min_rank()` taking `x` as the argument.
...
```{r general-transformations-2-solution, exercise.reveal_solution = FALSE}
x <- c(1, 2, 2, 3, 4, NA)
min_rank(x)
```

```{r general-transformations-2-check}
grade_this_code()
```
...
```{r general-transformations-3-solution, exercise.reveal_solution = FALSE}
x <-  c(1, 2, 2, 3, 4, NA)
min_rank(desc(x))
```

```{r general-transformations-3-check}
grade_this_code()
```
...
Type in `ranktypes` and hit "Submit Answer".
...
```{r general-transformations-4-solution, exercise.reveal_solution = FALSE}
ranktypes
```

```{r general-transformations-4-check}
grade_this_code()
```
...
```{r general-transformations-5-solution, exercise.reveal_solution = FALSE}
ranktypes |> 
  mutate(row_number = row_number(x))
```

```{r general-transformations-5-check}
grade_this_code()
```
...
```{r general-transformations-6-solution, exercise.reveal_solution = FALSE}
ranktypes |> 
  mutate(
    row_number = row_number(x),
...
```{r general-transformations-6-check}
grade_this_code()
```
...
```{r general-transformations-7-solution, exercise.reveal_solution = FALSE}
ranktypes |>
  mutate(
    row_number = row_number(x),
    dense_rank = dense_rank(x),
...
```{r general-transformations-7-check}
grade_this_code()
```
...
```{r general-transformations-8-solution, exercise.reveal_solution = FALSE}
ranktypes |>
  mutate(
    row_number = row_number(x),
    dense_rank = dense_rank(x),
...
```{r general-transformations-8-check}
grade_this_code()
```
...
Type in `numbers` and hit "Submit Answer". 
...
```{r general-transformations-9-solution, exercise.reveal_solution = FALSE}
numbers
```

```{r general-transformations-9-check}
grade_this_code()
```
...
```{r general-transformations-10-solution, exercise.reveal_solution = FALSE}
numbers |> 
  mutate(
    row0 = row_number() - 1)
```

```{r general-transformations-10-check}
grade_this_code()
```
...
```{r general-transformations-11-solution, exercise.reveal_solution = FALSE}
numbers |> 
  mutate(
    row0 = row_number() - 1,
    three_groups = row0 %% 3
...
```{r general-transformations-11-check}
grade_this_code()
```
...
```{r general-transformations-12-solution, exercise.reveal_solution = FALSE}
numbers |> 
  mutate(
    row0 = row_number() - 1,
    three_groups = row0 %% 3,
...
```{r general-transformations-12-check}
grade_this_code()
```
...
```{r general-transformations-13-solution, exercise.reveal_solution = FALSE}
x <- c(2, 5, 11, 11, 19, 35)
```

```{r general-transformations-13-check}
grade_this_code()
```
...
```{r general-transformations-14-solution, exercise.reveal_solution = FALSE}
x <- c(2, 5, 11, 11, 19, 35)
lag(x)
```

```{r general-transformations-14-check}
grade_this_code()
```
...
```{r general-transformations-15-solution, exercise.reveal_solution = FALSE}
x <- c(2, 5, 11, 11, 19, 35)
lead(x)
```

```{r general-transformations-15-check}
grade_this_code()
```

### Exercise 16

On the next line, subtract `lag(x)` from  `x`.
...
```{r general-transformations-16-solution, exercise.reveal_solution = FALSE}
x <- c(2, 5, 11, 11, 19, 35)
x - lag(x)
```

```{r general-transformations-16-check}
grade_this_code()
```
...
Type in `x == lag(x)` and hit "Submit Answer".
...
```{r general-transformations-17-solution, exercise.reveal_solution = FALSE}
x <- c(2, 5, 11, 11, 19, 35)
x == lag(x)
```

```{r general-transformations-17-check}
grade_this_code()
```
...
Type in `times_visited` and hit "Submit Answer".
...
```{r general-transformations-18-solution, exercise.reveal_solution = FALSE}
times_visited
```

```{r general-transformations-18-check}
grade_this_code()
```
...
```{r general-transformations-19-solution, exercise.reveal_solution = FALSE}
times_visited |> 
  mutate(diff = time - lag(time, default = first(time)))
```

```{r general-transformations-19-check}
grade_this_code()
```
...
```{r general-transformations-20-solution, exercise.reveal_solution = FALSE}
times_visited |> 
  mutate(
    diff = time - lag(time, default = first(time)), 
    has_gap = diff >= 5
...
```{r general-transformations-20-check}
grade_this_code()
```
...
```{r general-transformations-21-solution, exercise.reveal_solution = FALSE}
times_visited |>
  mutate(
    diff = time - lag(time, default = first(time)), 
    has_gap = diff >= 5,
...
```{r general-transformations-21-check}
grade_this_code()
```
...
Imagine you have a dataframe with a bunch of repeated values. Type in `repetition` and hit "Submit Answer".
...
```{r general-transformations-22-solution, exercise.reveal_solution = FALSE}
repetition
```

```{r general-transformations-22-check}
grade_this_code()
```
...
```{r general-transformations-23-solution, exercise.reveal_solution = FALSE}
repetition |> 
  mutate(id = consecutive_id(x))
```

```{r general-transformations-23-check}
grade_this_code()
```
...
```{r general-transformations-24-solution, exercise.reveal_solution = FALSE}
repetition |> 
  mutate(id = consecutive_id(x)) |> 
  slice_head(n = 1)
```

```{r general-transformations-24-check}
grade_this_code()
```
...
```{r general-transformations-25-solution, exercise.reveal_solution = FALSE}
repetition |> 
  mutate(id = consecutive_id(x)) |> 
  slice_head(n = 1, by = id)
```

```{r general-transformations-25-check}
grade_this_code()
```
...
```{r numeric-summaries-1-solution, exercise.reveal_solution = FALSE}
flights |> 
  summarize(
...
```{r numeric-summaries-2-solution, exercise.reveal_solution = FALSE}
flights |> 
  summarize(
    mean = mean(dep_delay, na.rm = TRUE),
...
```{r numeric-summaries-3-solution, exercise.reveal_solution = FALSE}
flights |> 
  summarize(
    mean = mean(dep_delay, na.rm = TRUE),
    median = median(dep_delay, na.rm = TRUE),
...
```{r numeric-summaries-4-solution, exercise.reveal_solution = FALSE}
flights |> 
  summarize(
    mean = mean(dep_delay, na.rm = TRUE),
    median = median(dep_delay, na.rm = TRUE),
...
```{r numeric-summaries-5-solution, exercise.reveal_solution = FALSE}
flights |> 
  summarize(
    mean = mean(dep_delay, na.rm = TRUE),
    median = median(dep_delay, na.rm = TRUE),
...
```{r numeric-summaries-6-solution, exercise.reveal_solution = FALSE}
flights |> 
  summarize(
    mean = mean(dep_delay, na.rm = TRUE),
    median = median(dep_delay, na.rm = TRUE),
...
```{r numeric-summaries-7-solution, exercise.reveal_solution = FALSE}
flights |> 
  summarize(
    mean = mean(dep_delay, na.rm = TRUE),
    median = median(dep_delay, na.rm = TRUE),
...
```{r numeric-summaries-8-solution, exercise.reveal_solution = FALSE}
flights |> 
  summarize(
    mean = mean(dep_delay, na.rm = TRUE),
    median = median(dep_delay, na.rm = TRUE),
...
```{r numeric-summaries-9-solution, exercise.reveal_solution = FALSE}
flights |> 
  summarize(
    mean = mean(dep_delay, na.rm = TRUE),
    median = median(dep_delay, na.rm = TRUE),
...
```{r numeric-summaries-10-solution, exercise.reveal_solution = FALSE}
flights |> 
  summarize(
    mean = mean(dep_delay, na.rm = TRUE),
    median = median(dep_delay, na.rm = TRUE),
...
```{r numeric-summaries-11-solution, exercise.reveal_solution = FALSE}
flights |> 
  summarize(
...
```{r numeric-summaries-12-solution, exercise.reveal_solution = FALSE}
flights |> 
  summarize(
    max = max(dep_delay, na.rm = TRUE),
...
```{r numeric-summaries-13-solution, exercise.reveal_solution = FALSE}
flights |> 
  summarize(
    max = max(dep_delay, na.rm = TRUE),
    q95 = quantile(dep_delay, 0.95, na.rm = TRUE),
...
```{r numeric-summaries-14-solution, exercise.reveal_solution = FALSE}
flights |> 
  summarize(
...
```{r numeric-summaries-15-solution, exercise.reveal_solution = FALSE}
flights |> 
  summarize(
    distance_sd = IQR(distance),
    n=n()
...
```{r numeric-summaries-16-solution, exercise.reveal_solution = FALSE}
flights |> 
  summarize(
    distance_sd = IQR(distance),
    n=n(),
...
```{r numeric-summaries-17-solution, exercise.reveal_solution = FALSE}
flights |> 
  summarize(
    distance_sd = IQR(distance),
    n=n(),
...
```{r numeric-summaries-19-solution, exercise.reveal_solution = FALSE}
flights |> 
  filter(dep_delay < 120)
```

```{r numeric-summaries-19-check}
grade_this_code()
```
...
```{r numeric-summaries-20-hint-2, eval = FALSE}
flights |> 
  filter(dep_delay < 120) |> 
  ggplot(aes(x = dep_delay, group = interaction(day, month)))
```

```{r numeric-summaries-20-solution, exercise.reveal_solution = FALSE}
flights |> 
  filter(dep_delay < 120) |> 
  ggplot(aes(x = dep_delay, group = interaction(day, month)))
```

```{r numeric-summaries-20-check}
grade_this_code()
```
...
```{r numeric-summaries-21-solution, exercise.reveal_solution = FALSE}
flights |> 
  filter(dep_delay < 120) |> 
  ggplot(aes(x = dep_delay, group = interaction(day, month))) +
...
```{r numeric-summaries-22-solution, exercise.reveal_solution = FALSE}
flights |> 
  summarize(
...
```{r numeric-summaries-23-solution, exercise.reveal_solution = FALSE}
flights |> 
  summarize(
    first_dep = first(dep_time, na_rm = TRUE),
...
```{r numeric-summaries-24-solution, exercise.reveal_solution = FALSE}
flights |> 
  summarize(
    first_dep = first(dep_time, na_rm = TRUE),
    fifth_dep = nth(dep_time, 5, na_rm = TRUE),
...
```{r numeric-summaries-25-solution, exercise.reveal_solution = FALSE}
flights |> 
  summarize(
    first_dep = first(dep_time, na_rm = TRUE),
    fifth_dep = nth(dep_time, 5, na_rm = TRUE),
...
```{r numeric-summaries-26-solution, exercise.reveal_solution = FALSE}
flights |> 
  mutate(y = min_rank(sched_dep_time))
```

```{r numeric-summaries-26-check}
grade_this_code()
```
...
```{r numeric-summaries-27-solution, exercise.reveal_solution = FALSE}
flights |> 
  mutate(y = min_rank(sched_dep_time),
         .by = c(year, month, day))
```

```{r numeric-summaries-27-check}
grade_this_code()
```
...
```{r numeric-summaries-28-solution, exercise.reveal_solution = FALSE}
flights |> 
  mutate(y=min_rank(sched_dep_time),
         .by = c(year, month, day)) |>
