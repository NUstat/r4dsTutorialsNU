...
<!-- Modification of r4ds.tutorials author: David Kane and Melissa Ban-->
...
```{r modifying-multiple-columns-1-solution, exercise.reveal_solution = FALSE}
library(tidyverse)
```

```{r modifying-multiple-columns-1-check}
grade_this_code()
```
...
Hit "Submit Answer" to create the `df` tibble.
...
```{r modifying-multiple-columns-2-solution, exercise.reveal_solution = FALSE}
df <- tibble(
  a = rnorm(10),
  b = rnorm(10),
  c = rnorm(10),
...
```{r modifying-multiple-columns-2-check}
grade_this_code()
```
...
```{r modifying-multiple-columns-3-solution, exercise.reveal_solution = FALSE}
df
```

```{r modifying-multiple-columns-3-check}
grade_this_code()
```
...
```{r modifying-multiple-columns-4-solution, exercise.reveal_solution = FALSE}
...
```{r modifying-multiple-columns-4-check}
grade_this_code()
```
...
```{r modifying-multiple-columns-5-solution, exercise.reveal_solution = FALSE}
# option 1 ----
df |> 
  summarise(n = n())
...
```{r modifying-multiple-columns-5-check}
grade_this_code()
```
...
```{r modifying-multiple-columns-6-solution, exercise.reveal_solution = FALSE}
# option 1 ----
df |>
  summarise(
    n = n(),
...
```{r modifying-multiple-columns-6-check}
grade_this_code()
```
...
```{r modifying-multiple-columns-7-solution, exercise.reveal_solution = FALSE}
# option 1 ----
df |>
  summarise(
    n = n(),
...
```{r modifying-multiple-columns-7-check}
grade_this_code()
```
...
```{r modifying-multiple-columns-8-solution, exercise.reveal_solution = FALSE}
# option 1 ----
df |> 
  summarise(n = n())
...
```{r modifying-multiple-columns-8-check}
grade_this_code()
```
...
```{r modifying-multiple-columns-9-solution, exercise.reveal_solution = FALSE}
# option 1 ----
df |>
  summarise(
    n = n(),
...
```{r modifying-multiple-columns-9-check}
grade_this_code()
```
...
Hit "Submit Answer" to create `df2`.
...
```{r selecting-columns-with-cols-1-solution, exercise.reveal_solution = FALSE}
df2 <- tibble(
  grp = sample(1:2, 10, replace = TRUE),
  a = rnorm(10),
  b = rnorm(10),
...
```{r selecting-columns-with-cols-1-check}
grade_this_code()
```
...
```{r selecting-columns-with-cols-2-solution, exercise.reveal_solution = FALSE}
# option 1 ----
df2 |> 
  summarise()
...
```{r selecting-columns-with-cols-2-check}
grade_this_code()
```
...
```{r selecting-columns-with-cols-3-solution, exercise.reveal_solution = FALSE}
# option 1 ----
df2 |> 
  summarise(.by = grp)
...
```{r selecting-columns-with-cols-3-check}
grade_this_code()
```

### 

Because the `.cols` arguement uses the same specifications as `select()`, so you can use functions like `starts_with()` and `ends_with()` to select columns based on their name.
...
```{r selecting-columns-with-cols-4-solution, exercise.reveal_solution = FALSE}
# option 1 ----
df2 |>
  summarise(
    .by = grp,
...
```{r selecting-columns-with-cols-4-check}
grade_this_code()
```
...
```{r selecting-columns-with-cols-5-solution, exercise.reveal_solution = FALSE}
rnorm_na <- function(n, n_na, mean = 0, sd = 1) {
  sample(c(rnorm(n - n_na, mean = mean, sd = sd), rep(NA, n_na)))
}
...
```{r selecting-columns-with-cols-5-check}
grade_this_code()
```
...
```{r selecting-columns-with-cols-6-solution, exercise.reveal_solution = FALSE}
# option 1 ----
df_miss |> 
  summarize()
...
```{r selecting-columns-with-cols-6-check}
grade_this_code()
```
...
Add `across(a:d, median)` and `n = n()` to the call to `summarize()`.
...
```{r selecting-columns-with-cols-7-solution, exercise.reveal_solution = FALSE}
# option 1 ----
df_miss |>
...
```{r selecting-columns-with-cols-7-check}
grade_this_code()
```
...
```{r selecting-columns-with-cols-8-solution, exercise.reveal_solution = FALSE}
# option 1 ----
df_miss |>
  summarize(
    across(
...
```{r selecting-columns-with-cols-8-check}
grade_this_code()
```
...
```{r selecting-columns-with-cols-9-solution, exercise.reveal_solution = FALSE}
# option 1 ----
df_miss |>
  summarize(
    across(
...
```{r selecting-columns-with-cols-9-check}
grade_this_code()
```
...
```{r selecting-columns-with-cols-10-solution, exercise.reveal_solution = FALSE}
# option 1 ---- 
df_miss |>
  summarize(across(a:d, list()),
    n = n()
  )
...
```{r selecting-columns-with-cols-10-check}
grade_this_code()
```
...
```{r selecting-columns-with-cols-11-solution, exercise.reveal_solution = FALSE}
# option 1 ----
df_miss |>
  summarize(
    across(
...
```{r selecting-columns-with-cols-11-check}
grade_this_code()
```
...
```{r selecting-columns-with-cols-12-solution, exercise.reveal_solution = FALSE}
# option 1 ----
df_miss |>
  summarize(
    across(
...
```{r selecting-columns-with-cols-12-check}
grade_this_code()
```
...
```{r column-names-1-solution, exercise.reveal_solution = FALSE}
df_miss |>
  summarize(
    across(
      a:d,
...
```{r column-names-2-solution, exercise.reveal_solution = FALSE}
df_miss |>
  summarize(
    across(a:d,
      list(
...
```{r column-names-3-solution, exercise.reveal_solution = FALSE}
df_miss |> 
  mutate()
```

```{r column-names-3-check}
grade_this_code()
```
...
```{r column-names-4-solution, exercise.reveal_solution = FALSE}
df_miss |>
  mutate(
    across(
      a:d,
...
```{r column-names-5-solution, exercise.reveal_solution = FALSE}
df_miss |>
  mutate(
    across(
      a:d, 
...
```{r column-names-6-solution, exercise.reveal_solution = FALSE}
df_miss |> 
  filter()
```

```{r column-names-6-check}
grade_this_code()
```
...
```{r column-names-7-solution, exercise.reveal_solution = FALSE}
df_miss |> 
  filter(if_any())
```

```{r column-names-7-check}
grade_this_code()
```
...
```{r column-names-8-solution, exercise.reveal_solution = FALSE}
df_miss |> 
  filter(if_any(a:d, is.na))
```

```{r column-names-8-check}
grade_this_code()
```
...
```{r column-names-9-solution, exercise.reveal_solution = FALSE}
df_miss |> 
  filter(if_all(a:d, is.na))
```

```{r column-names-9-check}
grade_this_code()
```
...
```{r across-in-functions-1-solution, exercise.reveal_solution = FALSE}
expand_dates <- function(df) {
  df |> 
    mutate(
      across(where(is.Date), list(year = year, month = month, day = mday))
...
```{r across-in-functions-2-solution, exercise.reveal_solution = FALSE}
df_date |> 
  expand_dates()
```

```{r across-in-functions-2-check}
grade_this_code()
```
...
Hit "Submit Answer" to define the following function.
...
```{r across-in-functions-3-solution, exercise.reveal_solution = FALSE}
summarize_means <- function(df, summary_vars = where(is.numeric)) {
  df |> 
    summarize(
      across({{ summary_vars }}, \(x) mean(x, na.rm = TRUE)),
...
```{r across-in-functions-4-solution, exercise.reveal_solution = FALSE}
diamonds |> 
  group_by(cut) |> 
  summarize_means()
```

```{r across-in-functions-4-check}
grade_this_code()
```

### 

We mentioned that where possible, `.by` should be used instead of `group_by()`. However, this is a situation where it is not possible to use `.by`.
...
```{r across-in-functions-5-solution, exercise.reveal_solution = FALSE}
diamonds |> 
  group_by(cut) |> 
  summarize_means(c(carat, x:z))
```

```{r across-in-functions-5-check}
grade_this_code()
```
...
Hit "Submit Answer". Pay attention to the table generated.
...
```{r across-in-functions-6-solution, exercise.reveal_solution = FALSE}
df |>
  summarize(
    across(
      a:d,
...
Hit "Submit Answer" to create the following tibble.
...
```{r across-in-functions-7-solution, exercise.reveal_solution = FALSE}
df_paired <- tibble(
  a_val = rnorm(10),
  a_wts = runif(10),
  b_val = rnorm(10),
...
```{r across-in-functions-7-check}
grade_this_code()
```

### Exercise 8

Type in `df_paired` and hit "Submit Answer". 
...
```{r across-in-functions-8-solution, exercise.reveal_solution = FALSE}
df_paired
```

```{r across-in-functions-8-check}
grade_this_code()
```
...
Pipe `df_paired` into the function `pivot_longer()`. Add the argument `everything()` to the function `pivot_longer()`. 
...
```{r across-in-functions-9-solution, exercise.reveal_solution = FALSE}
df_paired |> 
  pivot_longer(everything())
```

```{r across-in-functions-9-check}
grade_this_code()
```
...
```{r across-in-functions-10-solution, exercise.reveal_solution = FALSE}
df_paired |>
  pivot_longer(everything(),
    names_to = c("group", ".value"),
    names_sep = "_"
...
```{r across-in-functions-10-check}
grade_this_code()
```
...
```{r reading-multiple-files-1-solution, exercise.reveal_solution = FALSE}
paths <- list.files("data/gapminder", pattern = "[.]xlsx$", full.names = TRUE)

paths
```

```{r reading-multiple-files-1-check}
grade_this_code()
```
...
Type `files[[3]]` and hit "Submit Answer", see what happens. Note that we are using `[[]]` here instead of `()`.
...
```{r reading-multiple-files-2-solution, exercise.reveal_solution = FALSE}
files[[3]]
```

```{r reading-multiple-files-2-check}
grade_this_code()
```
...
Type `map()` in the code chunk, then add argument `paths` and `readxl::read_excel` to the call to `map()`.
...
```{r reading-multiple-files-3-solution, exercise.reveal_solution = FALSE}
map(paths, readxl::read_excel)
```

```{r reading-multiple-files-3-check}
grade_this_code()
```
...
Type `length()` and add `files`, the collection of tibbles we just created, as the argument.
...
```{r reading-multiple-files-4-solution, exercise.reveal_solution = FALSE}
length(files)
```

```{r reading-multiple-files-4-check}
grade_this_code()
```
...
Submit the code below to see what happens.

```{r reading-multiple-files-5, exercise = TRUE}
files
```

```{r reading-multiple-files-5-solution, exercise.reveal_solution = FALSE}
files
```

```{r reading-multiple-files-5-check}
grade_this_code()
```
...
Type `list_rbind()` and add `files` as the argument.
...
```{r reading-multiple-files-6-solution, exercise.reveal_solution = FALSE}
list_rbind(files)
```

```{r reading-multiple-files-6-check}
grade_this_code()
```
...
Pipe `paths` into function `map()`.

Then add `\(path) readxl::read_excel(path, n_max = 1)` into the function `map()`.

```{r reading-multiple-files-7, exercise = TRUE}

```

```{r reading-multiple-files-7-hint-1, eval = FALSE}
paths |> 
  map(\(path) readxl::read_excel(path, n_max = 1))
```

```{r reading-multiple-files-7-solution, exercise.reveal_solution = FALSE}
paths |> 
  map(\(path) readxl::read_excel(path, n_max = 1))
```

```{r reading-multiple-files-7-check}
grade_this_code()
```
...
```{r reading-multiple-files-8-solution, exercise.reveal_solution = FALSE}
paths |> 
  map(\(path) readxl::read_excel(path, n_max = 1)) |> 
  list_rbind()
```

```{r reading-multiple-files-8-check}
grade_this_code()
```
...
```{r reading-multiple-files-9-solution, exercise.reveal_solution = FALSE}
paths |> set_names(basename) 
```

```{r reading-multiple-files-9-check}
grade_this_code()
```
...
```{r reading-multiple-files-10-solution, exercise.reveal_solution = FALSE}
paths |> 
  set_names(basename) |> 
  map(readxl::read_excel)
```

```{r reading-multiple-files-10-check}
grade_this_code()
```
...
Assign the previous code to `files`.
...
```{r reading-multiple-files-11-solution, exercise.reveal_solution = FALSE}
files <- paths |> 
  set_names(basename) |> 
  map(readxl::read_excel)
```

```{r reading-multiple-files-11-check}
grade_this_code()
```
...
Type `files[[""]]`, with `1962.xlsx` between the `""`

```{r reading-multiple-files-12-setup}
files <- paths |> 
  set_names(basename) |> 
  map(readxl::read_excel)
```
...
```{r reading-multiple-files-12-solution, exercise.reveal_solution = FALSE}
files[["1962.xlsx"]]
```

```{r reading-multiple-files-12-check}
grade_this_code()
```

### 

You should get the file of the respective year 1962! 

### Exercise 13

Pipe the code provided into `list_rbind() `, then add argument `names_to = "year"` to this function.
...
```{r reading-multiple-files-13-solution, exercise.reveal_solution = FALSE}
paths |> 
  set_names(basename) |> 
  map(readxl::read_excel) |> 
  list_rbind(names_to = "year")
...
```{r reading-multiple-files-13-check}
grade_this_code()
```
...
```{r reading-multiple-files-14-solution, exercise.reveal_solution = FALSE}
paths |> 
  set_names(basename) |> 
  map(readxl::read_excel) |> 
  list_rbind(names_to = "year") |> 
...
```{r reading-multiple-files-14-check}
grade_this_code()
```
...
Assign the previous code to `gapminder`
...
```{r reading-multiple-files-15-solution, exercise.reveal_solution = FALSE}
gapminder <- paths |> 
  set_names(basename) |> 
  map(readxl::read_excel) |> 
  list_rbind(names_to = "year") |> 
...
```{r reading-multiple-files-15-check}
grade_this_code()
```
...
Type `write_csv()` in the empty code chunk below, hit "Submit Answer".
...
```{r save-your-work-2-solution, exercise.reveal_solution = FALSE}
write_csv(gapminder, "gapminder.csv")
```

```{r save-your-work-2-check}
grade_this_code()
```
...
<!-- ```{r save-your-work-3-solution, exercise.reveal_solution = FALSE} -->
<!-- df <- read_csv(paths) -->
<!-- ``` -->

<!-- ```{r save-your-work-3-check} -->
<!-- grade_this_code() -->
<!-- ``` -->
...
<!-- ```{r save-your-work-4-solution, exercise.reveal_solution = FALSE} -->
<!-- df |>  -->
<!--     filter(!is.na(id)) -->
<!-- ``` -->

<!-- ```{r save-your-work-4-check} -->
<!-- grade_this_code() -->
<!-- ``` -->
...
<!-- ```{r save-your-work-5-solution, exercise.reveal_solution = FALSE} -->
<!-- df |>  -->
<!--     filter(!is.na(id)) |>  -->
<!--     mutate(id = tolower(id)) -->
<!-- ``` -->

<!-- ```{r save-your-work-5-check} -->
<!-- grade_this_code() -->
<!-- ``` -->
...
<!-- ```{r save-your-work-6-solution, exercise.reveal_solution = FALSE} -->
<!-- df |>  -->
<!--   filter(!is.na(id)) |>  -->
<!--   mutate(id = tolower(id)) |>  -->
<!--   pivot_longer(jan:dec, names_to = "month") -->
...
<!-- Assigned variable name `process_file` to <- `function(path)`, followed by a `{}`. -->

<!-- Within the `{}` type `df <- read_csv(path)` followed by the previous code.  -->
...
<!-- ```{r save-your-work-7-solution, exercise.reveal_solution = FALSE} -->
<!-- process_file <- function(path) { -->
<!--   df <- read_csv(path) -->
...
<!-- Pipe `paths` into `map()` and add `process_file` as the argument to `map()`. -->
...
<!-- ```{r save-your-work-8-solution, exercise.reveal_solution = FALSE} -->
<!-- paths |>  -->
<!--   map(process_file) -->
<!-- ``` -->

<!-- ```{r save-your-work-8-check} -->
<!-- grade_this_code() -->
<!-- ``` -->
...
<!-- ```{r save-your-work-9-solution, exercise.reveal_solution = FALSE} -->
<!-- paths |>  -->
<!--   map(process_file) |>  -->
<!--   list_rbind() -->
<!-- ``` -->

<!-- ```{r save-your-work-9-check} -->
<!-- grade_this_code() -->
<!-- ``` -->
...
<!-- ```{r save-your-work-10-solution, exercise.reveal_solution = FALSE} -->
<!-- paths |>  -->
<!--   map(readxl::read_excel) -->
<!-- ``` -->

<!-- ```{r save-your-work-10-check} -->
<!-- grade_this_code() -->
<!-- ``` -->
...
<!-- Create a tibble using the `tibble()` function. Add the argument `col_name` with value `names(df)` to the call to `tibble()`. -->
...
<!-- ```{r save-your-work-11-solution, exercise.reveal_solution = FALSE} -->
<!-- tibble( -->
<!--     col_name = names(df)) -->
<!-- ``` -->

<!-- ```{r save-your-work-11-check} -->
<!-- grade_this_code() -->
<!-- ``` -->
...
<!-- Now, add the second argument, `col_type`, then set its value to be `map_chr(df, vctrs::vec_ptype_full)`. -->
...
<!-- ```{r save-your-work-12-solution, exercise.reveal_solution = FALSE} -->
<!-- tibble( -->
<!--     col_name = names(df),  -->
<!--     col_type = map_chr(df, vctrs::vec_ptype_full)) -->
<!-- ``` -->

<!-- ```{r save-your-work-12-check} -->
<!-- grade_this_code() -->
<!-- ``` -->
...
<!-- ```{r save-your-work-13-solution, exercise.reveal_solution = FALSE} -->
<!-- tibble( -->
<!--     col_name = names(df),  -->
<!--     col_type = map_chr(df, vctrs::vec_ptype_full), -->
<!--     n_miss = map_int(df, \(x) sum(is.na(x)))) -->
...
<!-- ```{r save-your-work-14-solution, exercise.reveal_solution = FALSE} -->
<!-- function(df) { -->
<!--   tibble( -->
<!--     col_name = names(df),  -->
<!--     col_type = map_chr(df, vctrs::vec_ptype_full), -->
...
<!-- ```{r save-your-work-15-solution, exercise.reveal_solution = FALSE} -->
<!-- df_types <- function(df) { -->
<!--   tibble( -->
<!--     col_name = names(df),  -->
<!--     col_type = map_chr(df, vctrs::vec_ptype_full), -->
...
<!-- Type `df_types()` and an argument `gapminder`, hit "Submit Answer", see what happens. -->
...
<!-- ```{r save-your-work-16-solution, exercise.reveal_solution = FALSE} -->
<!-- df_types(gapminder) -->
<!-- ``` -->

<!-- ```{r save-your-work-16-check} -->
<!-- grade_this_code() -->
<!-- ``` -->
...
<!-- ```{r save-your-work-17-solution, exercise.reveal_solution = FALSE} -->
<!-- paths |>  -->
<!--   map() -->
<!-- ``` -->

<!-- ```{r save-your-work-17-check} -->
<!-- grade_this_code() -->
<!-- ``` -->
...
<!-- ```{r save-your-work-18-solution, exercise.reveal_solution = FALSE} -->
<!-- paths |>  -->
<!--   map(possibly()) -->
<!-- ``` -->

<!-- ```{r save-your-work-18-check} -->
<!-- grade_this_code() -->
<!-- ``` -->
...
<!-- ```{r save-your-work-19-solution, exercise.reveal_solution = FALSE} -->
<!-- paths |>  -->
<!--   map(possibly(\(path) readxl::read_excel(path), NULL)) -->
<!-- ``` -->

<!-- ```{r save-your-work-19-check} -->
<!-- grade_this_code() -->
<!-- ``` -->
...
<!-- ```{r save-your-work-20-solution, exercise.reveal_solution = FALSE} -->
<!-- files <- paths |>  -->
<!--   map(possibly(\(path) readxl::read_excel(path), NULL)) -->
<!-- ``` -->

<!-- ```{r save-your-work-20-check} -->
<!-- grade_this_code() -->
<!-- ``` -->
...
<!-- ```{r save-your-work-21-solution, exercise.reveal_solution = FALSE} -->
<!-- files |> list_rbind() -->
<!-- ``` -->

<!-- ```{r save-your-work-21-check} -->
<!-- grade_this_code() -->
<!-- ``` -->
...
<!-- ```{r save-your-work-22-solution, exercise.reveal_solution = FALSE} -->
<!-- map_vec(files, is.null) -->
<!-- ``` -->

<!-- ```{r save-your-work-22-check} -->
<!-- grade_this_code() -->
<!-- ``` -->
...
<!-- ```{r save-your-work-23-solution, exercise.reveal_solution = FALSE} -->
<!-- failed <- map_vec(files, is.null) -->
<!-- ``` -->

<!-- ```{r save-your-work-23-check} -->
<!-- grade_this_code() -->
<!-- ``` -->
...
<!-- ```{r save-your-work-24-solution, exercise.reveal_solution = FALSE} -->
<!-- paths[failed] -->
<!-- ``` -->

<!-- ```{r save-your-work-24-check} -->
<!-- grade_this_code() -->
<!-- ``` -->
...
Sometimes when working with many files, it’s not possible to fit all your data into memory at once, and you can’t do `map(files, read_csv)`. One approach to deal with this problem is to load your data into a database. 

Submit the code in the code chunk below. 

```{r saving-multiple-outputs-1, exercise = TRUE}
con <- DBI::dbConnect(duckdb::duckdb())
```

```{r saving-multiple-outputs-1-solution, exercise.reveal_solution = FALSE}
con <- DBI::dbConnect(duckdb::duckdb())
```

```{r saving-multiple-outputs-1-check}
grade_this_code()
```
...
`con` is a connection to a database created by the [**duckdb**](https://r.duckdb.org/) package. 

<!-- Note that because of limits on **learnr** tutorials, we will need to run this command --- and any other commands which modify the database --- in each Exercise block for this tutorial. Apologies for the repetition! -->

### Exercise 2

We need to start by creating a table that we will fill in with data. The easiest way to do this is by creating a template, a dummy data frame that contains all the columns we want, but only a sampling of the data. Assign `readxl::read_excel(paths[[1]])` to `template`.
...
```{r saving-multiple-outputs-2-solution, exercise.reveal_solution = FALSE}
template <- readxl::read_excel(paths[[1]])
```

```{r saving-multiple-outputs-2-check}
grade_this_code()
```
...
Take the assignment from the previous question, add a pipe, and then use `mutate()` to add a new variable, `year`, and set it to 1952.
...
```{r saving-multiple-outputs-3-solution, exercise.reveal_solution = FALSE}
template <- readxl::read_excel(paths[[1]]) |> 
  mutate(year = 1952)
```

```{r saving-multiple-outputs-3-check}
grade_this_code()
```
...
```{r saving-multiple-outputs-4-solution, exercise.reveal_solution = FALSE}
con <- DBI::dbConnect(duckdb::duckdb())
DBI::dbCreateTable(con, "gapminder", template)
```

```{r saving-multiple-outputs-4-check}
grade_this_code()
```
...
Now, let's test it by piping `con` into `tbl("gapminder")`.

```{r saving-multiple-outputs-5-setup}
con <- DBI::dbConnect(duckdb::duckdb())
DBI::dbCreateTable(con, "gapminder", template)
```
...
```{r saving-multiple-outputs-5-solution, exercise.reveal_solution = FALSE}
con |> 
  tbl("gapminder")
```

```{r saving-multiple-outputs-5-check}
grade_this_code()
```
...
```{r saving-multiple-outputs-6-solution, exercise.reveal_solution = FALSE}
append_file <- function(path) {
  df <- readxl::read_excel(path)
  df$year <- parse_number(basename(path))
  DBI::dbAppendTable(con, "gapminder", df)
...
```{r saving-multiple-outputs-6-check}
grade_this_code()
```
...
```{r saving-multiple-outputs-7-setup}
con <- DBI::dbConnect(duckdb::duckdb())
DBI::dbCreateTable(con, "gapminder", template)
...
```{r saving-multiple-outputs-7-solution, exercise.reveal_solution = FALSE}
paths |>
 map(append_file)
```

```{r saving-multiple-outputs-7-check}
grade_this_code()
```
...
```{r saving-multiple-outputs-8-setup}
con <- DBI::dbConnect(duckdb::duckdb())
DBI::dbCreateTable(con, "gapminder", template)
...
```{r saving-multiple-outputs-8-solution, exercise.reveal_solution = FALSE}
paths |> 
  walk(append_file)
```

```{r saving-multiple-outputs-8-check}
grade_this_code()
``` 
...
```{r saving-multiple-outputs-9-setup}
con <- DBI::dbConnect(duckdb::duckdb())
DBI::dbCreateTable(con, "gapminder", template)
...
```{r saving-multiple-outputs-9-solution, exercise.reveal_solution = FALSE}
con |> 
  tbl("gapminder")
```

```{r saving-multiple-outputs-9-check}
grade_this_code()
``` 
...
```{r saving-multiple-outputs-10-setup}
con <- DBI::dbConnect(duckdb::duckdb())
DBI::dbCreateTable(con, "gapminder", template)
...
```{r saving-multiple-outputs-10-solution, exercise.reveal_solution = FALSE}
con |> 
  tbl("gapminder") |> 
  count(year)
```

```{r saving-multiple-outputs-10-check}
grade_this_code()
``` 
...
```{r writing-csv-files-1-solution, exercise.reveal_solution = FALSE}
diamonds |> 
  group_nest()
```

```{r writing-csv-files-1-check}
grade_this_code()
``` 
...
```{r writing-csv-files-2-solution, exercise.reveal_solution = FALSE}
diamonds |> 
  group_nest(clarity)
```

```{r writing-csv-files-2-check}
grade_this_code()
``` 
...
```{r writing-csv-files-3-solution, exercise.reveal_solution = FALSE}
diamonds |> 
  group_nest(clarity) |> 
  mutate(path = str_glue("diamonds-{clarity}.csv"))
```

```{r writing-csv-files-3-check}
grade_this_code()
``` 
...
```{r writing-csv-files-4-solution, exercise.reveal_solution = FALSE}
by_clarity <- diamonds |> 
    group_nest(clarity) |> 
    mutate(path = str_glue("diamonds-{clarity}.csv"))
```

```{r writing-csv-files-4-check}
grade_this_code()
``` 
...
Type `by_clarity` and hit "Submit Answer".
...
```{r writing-csv-files-5-solution, exercise.reveal_solution = FALSE}
by_clarity
```

```{r writing-csv-files-5-check}
grade_this_code()
``` 
...
```{r writing-csv-files-6-solution, exercise.reveal_solution = FALSE}
by_clarity$data[[1]]
```

```{r writing-csv-files-6-check}
grade_this_code()
``` 
...
But that would be annoying, especially if there were hundreds of values for `clarity`. Type `?walk2` to the space below. Hit "Submit Answer".
...
```{r writing-csv-files-7-solution, exercise.reveal_solution = FALSE}
?walk2
```

```{r writing-csv-files-7-check}
grade_this_code()
``` 
...
```{r writing-csv-files-8-solution, exercise.reveal_solution = FALSE}
walk2(by_clarity$data, by_clarity$path, write_csv)
```

```{r writing-csv-files-8-check}
grade_this_code()
``` 
...
<!-- ```{r writing-csv-files-9-solution, exercise.reveal_solution = FALSE} -->
<!-- list.files(pattern = "*.csv") -->
<!-- ``` -->

<!-- ```{r writing-csv-files-9-check} -->
<!-- grade_this_code() -->
<!-- ```  -->
...
<!-- ```{r writing-csv-files-10-solution, exercise.reveal_solution = FALSE} -->
<!-- list.files(pattern = "*.csv") |>  -->
<!--   file.remove() -->
<!-- ``` -->

<!-- ```{r writing-csv-files-10-check} -->
<!-- grade_this_code() -->
<!-- ```  -->
...
Below is a new function that draws the plot we want. Hit "Submit Answer".
...
Assign code above to `by_clarity`.
...
Type `walk2()` then add arguments `by_clarity$path`, `by_clarity$plot` and `\(path, plot) ggsave(path, plot)`.

```{r saving-plots-4-setup}
by_clarity <- by_clarity |> 
  mutate(
    plot = map(data, carat_histogram),
    path = str_glue("clarity-{clarity}.png")
...
```{r saving-plots-5-solution, exercise.reveal_solution = FALSE}
walk2(
  by_clarity$path,
  by_clarity$plot,
  \(path, plot) ggsave(path, plot, width = 6, height = 6)
