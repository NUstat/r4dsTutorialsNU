...
Assign a new variable `files` to the previous code.
...
```{r save-your-work-1-solution, exercise.reveal_solution = FALSE}
write_csv()
```

```{r save-your-work-1-check}
grade_this_code()
```
...
```{r saving-plots-6-solution, exercise.reveal_solution = FALSE}
walk2(
  by_clarity$path,
  by_clarity$plot,
  \(path, plot) ggsave(path, plot, width = 6, height = 6)
