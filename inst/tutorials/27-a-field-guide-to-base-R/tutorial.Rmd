...
<!-- Modification of r4ds.tutorials authors: Luit Deka and David Kane-->
...
for(i in seq_along(paths)){
  files.2[[i]] <- readxl::read_excel(paths[i])
}
```
...
Run the code below to create a tibble with three columns: `x`, `y`, and `z`. For `x`, the vector consists of the values `2, 3, 1, 1, NA`. For `y`, it contains letters from a to e. While `z` is equal to `runif(5)`. The tibble is assigned `df0`.
...
```{r subsetting-with-double-brackes-10-hint, eval=FALSE}
... <- tibble(...)
...$x
```
...
Using the `list()` function, create a list. 

1. Set the first list element to `a`, which is equal to a range of integers from 1 to 3. 
2. Set the second list element to `b`, which is equal to the string `"a string"`. 
3. Set the third list element to `c`, which is equal to `pi`, and set the fourth list element to `d`, which is equal to another list containing `-1` and `-5`. 
5. Then, assign the list `list1`. 
6. Type `list1` on the next line to see the result.
...
In the code chunk below, type in `str()`. Inside the function, type in `list1`, followed by double brackets, `[[]]`. Inside the double brackets, type in `4`. This will extract the list `d`. 

We can also replace the `4` with a `"d"` and obtain the same result because `[[]]` works with both variable names as well as the element number. 
...
In the code chunk below, type in `str()`, placing `list1$d` inside the function.
...
The most important member of this family is `lapply()`. Run `?lapply()` in the console and read the [Description]{.underline} section. Copy and paste the description of `lapply()`.
...
The function `sapply()` is very similar to `lapply()`. Run `?sapply()` in the console and read the [Description]{.underline} section. Copy and paste the description of `sapply()`.
...
Create a tibble, `df4`, where `a = 1`, `b = 2`, `c= "a"`, `d = "b"` and `e = 4` by running the code block.
...
Add a `for()` loop below the code. The argument in the `()` after `for` should be `i in seq_along(paths)`. The code in the body of the `for()` loop should be `files.2[[i]] <- readxl::read_excel(paths[i])`. After trhe for loop, type `files.2` to print it.
...
The trickiest part of this code is that we have `files.2[[i]]` with two pairs of nested brackets and `paths[i]` with only one pair. The reason for the difference is that `paths` is a simple vector. We just need one pair of brackets to access each element of a vector. `files.2`, on the other hand, is a list and, therefore, needs nested brackets.

### Exercise 12

To combine the list of tibbles into a single tibble you can use `do.call()` and `rbind()`. Run `do.call()` with two arguments: `rbind` and `files.2`.
