...
In the `diamonds` dataset, diamonds with a width less than 3 or greater than 20 are "unusual". Let's replace them with `NA`.

First, create a new variable called `diamonds2` and set it equal to `diamonds`.
...
Create a scatter plot with our `diamonds2` dataset, within the `aes()` function set `x` equal to `x` and `y` equal to `y_new`.
...
In the dataset `nycflights13::flights`, missing values in the `dep_time` variable indicate that the flight was cancelled. So you might want to compare the scheduled departure times for cancelled and non-cancelled times. You can do this by making a new variable, using `is.na()` to check if `dep_time` is missing.
...
Then within `aes()`, set the `x` argument to the `sched_dep_time` variable.
...
Add `labs()` to the pipeline and set the `title`, `subtitle`, `x` axis title, `y` axis title, and `color` label as follows:
...
Replace the call to `geom_point()` with `geom_bin2d()`.
