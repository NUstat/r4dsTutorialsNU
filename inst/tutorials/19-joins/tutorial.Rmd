...
As a example type `airlines` and submit.
...
```{r keys-1-solution, exercise.reveal_solution = FALSE}
airlines
```
...
Type `airports` and click "Submit Answer".
...
```{r keys-2-solution, exercise.reveal_solution = FALSE}
airports
```
...
```{r keys-3-solution, exercise.reveal_solution = FALSE}
planes
```
...
```{r keys-4-solution, exercise.reveal_solution = FALSE}
weather
```
...
Type `flights$tailnum` and submit it.
...
```{r keys-5-solution, exercise.reveal_solution = FALSE}
flights$tailnum
```
...
Type `flights$carrier` and submit it.
...
```{r keys-6-solution, exercise.reveal_solution = FALSE}
flights$carrier
```
...
Type `flights$origin` and submit it.
...
```{r keys-7-solution, exercise.reveal_solution = FALSE}
flights$origin
```
...
`flights$origin` is a foreign key that corresponds to the primary key `faa` in the `airports` tibble.
...
Start a pipe with `planes` to the `count()` function and within count set the argument as `tailnum`. Continue the pipe to `filter()` with `n > 1` as its input. This will filter out any elements where the count (n) is not greater than 1.
...
```{r keys-8-solution, exercise.reveal_solution = FALSE}
planes |> 
  count(tailnum) |> 
  filter(n > 1)
```
...
```{r keys-9-solution, exercise.reveal_solution = FALSE}
weather |> 
...
```{r keys-10-solution, exercise.reveal_solution = FALSE}
planes |> 
...
```{r keys-11-solution, exercise.reveal_solution = FALSE}
weather |> 
...
```{r keys-12-solution, exercise.reveal_solution = FALSE}
flights |> 
...
```{r keys-13-solution, exercise.reveal_solution = FALSE}
airports |> 
  count(alt, lat) |> 
  filter(n > 1)
```
...
That said, we might be better off introducing a simple numeric surrogate key using the row number instead of confusing the airlines and customers by using three columns as the primary keys. 
...
```{r keys-14-solution, exercise.reveal_solution = FALSE}
flights |> 
  mutate(
    id = row_number(), 
    .before = 1
...
```{r basic-joins-1-solution, exercise.reveal_solution = FALSE}
flights2 <- flights |> 
  select(year, time_hour, origin, dest, tailnum,carrier)
...
```{r basic-joins-2-solution, exercise.reveal_solution = FALSE}
flights2 |>
  left_join(airlines)
```

```{r basic-joins-2-check}
grade_this_code()
```
...
```{r basic-joins-3-solution, exercise.reveal_solution = FALSE}
flights2 |> 
  left_join(weather |> select(origin, time_hour, temp, wind_speed))
```

```{r basic-joins-3-check}
grade_this_code()
```
...
```{r basic-joins-4-solution, exercise.reveal_solution = FALSE}
flights2 |> 
  left_join(planes |> select(tailnum, type, engines, seats))
```

```{r basic-joins-4-check}
grade_this_code()
```
...
To demonstrate this, start a pipe with `flights2` to `filter()` where you only select the records where the `tailnum` is equal to `N3ALAA`, which doesn't exist in `planes`. Continue the pipe to `left_join()` with `planes |> select(tailnum, type, engines, seats)` in side of it.
...
```{r basic-joins-5-solution, exercise.reveal_solution = FALSE}
flights2 |> 
  filter(tailnum == "N3ALAA") |> 
  left_join(planes |> select(tailnum, type, engines, seats))
```

```{r basic-joins-5-check}
grade_this_code()
```
...
```{r basic-joins-6-solution, exercise.reveal_solution = FALSE}
flights2 |>
  left_join(planes)
```

```{r basic-joins-6-check}
grade_this_code()
```
...
```{r basic-joins-7-solution, exercise.reveal_solution = FALSE}
flights2 |>
  left_join(planes, join_by(tailnum))
```

```{r basic-joins-7-check}
grade_this_code()
```
...
```{r basic-joins-8-solution, exercise.reveal_solution = FALSE}
flights2 |>
  left_join(airports, join_by(dest == faa))
```

```{r basic-joins-8-check}
grade_this_code()
```
...
```{r basic-joins-9-solution, exercise.reveal_solution = FALSE}
flights2 |>
  left_join(airports, join_by(origin == faa))
```

```{r basic-joins-9-check}
grade_this_code()
```

### 

If we compare both the tibbles we can see the difference in the `name` variable.
...
```{r basic-joins-10-solution, exercise.reveal_solution = FALSE}
airports |> 
  semi_join(flights2, join_by(faa == dest))
```

```{r basic-joins-10-check}
grade_this_code()
```
...
Let's find missing rows in `airports`. That is, let's find the destination airports `flights2` that do not exist in `airports`. We can do this by using `anti_join()`. 

First start a pipe with `flights2` to `anti_join()` which takes in `airports` as the joining tibble and we will tell it to join by `dest == faa`. Continue the pipe with `distint(dest)`, to see a list of airports that existed in `flights2` but not in `airports`.
...
```{r basic-joins-11-solution, exercise.reveal_solution = FALSE}
flights2 |> 
  anti_join(airports, join_by(dest == faa)) |> 
  distinct(dest)
```

```{r basic-joins-11-check}
grade_this_code()
```

### 

We found four rows/airports that are missing from `airports` but not in `flights2`.
...
```{r basic-joins-12-solution, exercise.reveal_solution = FALSE}
flights2 |> 
  anti_join(planes, join_by(tailnum)) |> 
  distinct(tailnum)
```

```{r basic-joins-12-check}
grade_this_code()
```
...
Click "Submit Answer" to create the tibbles we will use to demonstrate or visualize the images in the upcoming exercises.

```{r how-do-joins-work-1, exercise = TRUE}
x <- tribble(
  ~key, ~val_x,
     1, "x1",
     2, "x2",
...
```{r how-do-joins-work-1-solution, exercise.reveal_solution = FALSE}
x <- tribble(
  ~key, ~val_x,
     1, "x1",
     2, "x2",
...
```{r how-do-joins-work-9-solution, exercise.reveal_solution = FALSE}
df1 <- tibble(key = c(1, 2, 2), val_x = c("x1", "x2", "x3"))
df2 <- tibble(key = c(1, 2, 2), val_x = c("x1", "x2", "x3"))
```

```{r how-do-joins-work-9-check}
grade_this_code()
```
...
Pipe `df1` to `inner_join()` and pass in `df2`. Join the tibbles by `key` using `join_by()`.
...
```{r how-do-joins-work-10-solution, exercise.reveal_solution = FALSE}
df1 |> 
  inner_join(df2, join_by(key))
```

```{r how-do-joins-work-10-check}
grade_this_code()
```

### 

While the first row in df1 only matches one row in df2, the second and third rows both match two rows. This is sometimes called a many-to-many join, and will cause a warning to emit. If you are doing this deliberately, you can set relationship = "many-to-many", as the warning suggests.
...
```{r nonequi-joins-1-solution, exercise.reveal_solution = FALSE}
x |> 
  left_join(y, by = "key", keep = TRUE)
```

```{r nonequi-joins-1-check}
grade_this_code()
```
...
```{r nonequi-joins-4-solution, exercise.reveal_solution = FALSE}
df <- tibble(name = c("John", "Simon", "Tracy", "Max"))
```

```{r nonequi-joins-4-check}
grade_this_code()
```
...
```{r nonequi-joins-5-solution, exercise.reveal_solution = FALSE}
df <- tibble(name = c("John", "Simon", "Tracy", "Max"))
df |> 
  cross_join(df)
```

```{r nonequi-joins-5-check}
grade_this_code()
```
...
Click "Submit Answer" to see how this would work.
...
```{r nonequi-joins-7-solution, exercise.reveal_solution = FALSE}
df <- tibble(id = 1:4, name = c("John", "Simon", "Tracy", "Max"))
...
Rolling joins return the closest match instead of all satisfying the inequality. to have  rolling join add `closest()` to the inequality condition. For example, `join_by(closest(x <= y))` finds the smallest `y ≥ x`, and `join_by(closest(x > y))` finds the largest `y < x`.` 
...
```{r nonequi-joins-9-solution, exercise.reveal_solution = FALSE}
parties <- tibble(q = 1:4, party = ymd(c("2022-01-10", "2022-04-04", "2022-07-11", "2022-10-03")))
```

```{r nonequi-joins-9-check}
grade_this_code()
```
...
```{r nonequi-joins-10-solution, exercise.reveal_solution = FALSE}
employees <- tibble(
  name = sample(babynames::babynames$name, 100),
  birthday = ymd("2022-01-01") + (sample(365, 100, replace = TRUE) - 1)
)
...
```{r nonequi-joins-11-solution, exercise.reveal_solution = FALSE}
employees |> 
  left_join(parties, join_by(closest(birthday >= party)))
```

```{r nonequi-joins-11-check}
grade_this_code()
```
...
```{r nonequi-joins-12-solution, exercise.reveal_solution = FALSE}
employees |> 
  right_join(parties, join_by(closest(birthday >= party)))
```

```{r nonequi-joins-12-check}
grade_this_code()
```
...
Click "Submit Answer" to create print a new tibble, `parties2`.
...
```{r nonequi-joins-13-solution, exercise.reveal_solution = FALSE}
parties2 <- tibble(
  q = 1:4,
  party = ymd(c("2022-01-10", "2022-04-04", "2022-07-11", "2022-10-03")),
  start = ymd(c("2022-01-01", "2022-04-04", "2022-07-11", "2022-10-03")),
...
`between(x, y_lower, y_upper)` is short for `x >= y_lower` and `x <= y_upper`.
`within(x_lower, x_upper, y_lower, y_upper)` is short for `x_lower >= y_lower` and `x_upper <= y_upper`.
`overlaps(x_lower, x_upper, y_lower, y_upper)` is short for `x_lower <= y_upper` and `x_upper >= y_lower`.
...
```{r nonequi-joins-14-solution, exercise.reveal_solution = FALSE}
parties2 |> 
  inner_join(parties2, join_by(overlaps(start, end, start, end), q < q))
```

```{r nonequi-joins-14-check}
grade_this_code()
```
...
When we ran the previous code, we see other info which we don't need, so let's remove those using `select()`. So copy the previous code and continue the pipe from `inner_join()` to `select()` and only select the `start.x, end.x, start.y, end.y`.
...
```{r nonequi-joins-15-solution, exercise.reveal_solution = FALSE}
parties2 |> 
  inner_join(parties2, join_by(overlaps(start, end, start, end), q < q)) |>
  select(start.x, end.x, start.y, end.y)
```

```{r nonequi-joins-15-check}
grade_this_code()
```
...
Click "Submit Answer" to create and print `parties3`. Note all we did was take `parties2` and changed the 2nd data in `end` to `"2022-07-10"`.
...
```{r nonequi-joins-16-solution, exercise.reveal_solution = FALSE}
parties3 <- tibble(
  q = 1:4,
  party = ymd(c("2022-01-10", "2022-04-04", "2022-07-11", "2022-10-03")),
  start = ymd(c("2022-01-01", "2022-04-04", "2022-07-11", "2022-10-03")),
...
```{r nonequi-joins-17-solution, exercise.reveal_solution = FALSE}
employees |> 
  inner_join(
    parties3, 
