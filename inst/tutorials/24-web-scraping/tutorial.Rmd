...
<!-- Perhaps mention the polite package. Or ethics and legalities? What about dynamic sites?  -->

<!-- DK: More detail in Introduction/Summary. -->
...
Look up the help page for the **rvest** package by running `help(package = "rvest")` in your command line. Copy/paste the title from the help page.  

```{r web-scaping-basics-2}
question_text(NULL,
	answer(NULL, correct = TRUE),
	allow_retry = TRUE,
	try_again_button = "Edit Answer",
...
We are using clues in the html formatting --- bolding, in this case --- to determine a substantive issue, what are the names of the characters. Nothing guarantees that this two things are connected. They don't have to be! And, if the html document itself is sloppy or inconsistent, they won't be. 
...
Continue the pipe with a call to `mutate()` which applies `str_squish()` on `rank_title_year`, assigning the result back to `rank_title_year`. Modifying a variable "in place" like this is a common approach. We don't need to keep the original value around. 
...
Modify the previous code by adding a second line within `mutate()` to create a new variable called `rank`, which is the result of running `parse_number()` on `rank_title_year`.
...
... |> 
  mutate(
    rank_title_year = str_squish(rank_title_year),
    rank = parse_number(rank_title_year)
  )
```
...
Add a third line within `mutate()` to create a new variable called `title`, which is the result of running `str_extract()` on `rank_title_year` with the pattern argument set to `"(?<=\\d\\.\\s)(.*)(?=\\s\\(\\d{4}\\))"`.
...
... |> 
  mutate(
    rank_title_year = str_squish(rank_title_year),
    rank = parse_number(rank_title_year),
    title = str_extract(rank_title_year, "(?<=\\d\\.\\s)(.*)(?=\\s\\(\\d{4}\\))")
  )
```
