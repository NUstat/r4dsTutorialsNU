...
```{r factor-basics-1-solution, exercise.reveal_solution = FALSE}
library(tidyverse)
```

```{r factor-basics-1-check}
grade_this_code()
```
...
Look up the help page for **forcats** by entering `help(package = "forcats")` at the Console.
...
Click "Submit Answer" to create the variable `x1`.

```{r factor-basics-2, exercise = TRUE}
x1 <- c("Dec", "Apr", "Jan", "Mar")
```

```{r factor-basics-2-solution, exercise.reveal_solution = FALSE}
x1 <- c("Dec", "Apr", "Jan", "Mar")
```

```{r factor-basics-2-check}
grade_this_code()
```

### 

Note that `x1` is a character or string variable. This can lead to problems because months are ordered and there are exactly 12 possible values. Months is a great example of a categorical/factor variable.
...
```{r factor-basics-3-hint-1, eval = FALSE}
sort(...)
```

```{r factor-basics-3-solution, exercise.reveal_solution = FALSE}
sort(x1)
```

```{r factor-basics-3-check}
grade_this_code()
```
...
Click "Submit Answer" to create the `x2` variable, another character vector. But note the misspelling of "Jan" as "Jam".

```{r factor-basics-4, exercise = TRUE}
x2 <- c("Dec", "Apr", "Jam", "Mar")
```

```{r factor-basics-4-solution, exercise.reveal_solution = FALSE}
x2 <- c("Dec", "Apr", "Jam", "Mar")
```

```{r factor-basics-4-check}
grade_this_code()
```
...
Click "Submit Answer" to create the `month_levels` variable.
...
```{r factor-basics-5-solution, exercise.reveal_solution = FALSE}
month_levels <- c(
  "Jan", "Feb", "Mar", "Apr", "May", "Jun", 
  "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"
)
...
```{r factor-basics-6-hint-1, eval = FALSE}
factor(x1, ... = month_levels)
```

```{r factor-basics-6-solution, exercise.reveal_solution = FALSE}
factor(x1, levels = month_levels)
```

```{r factor-basics-6-check}
grade_this_code()
```
...
```{r factor-basics-7-solution, exercise.reveal_solution = FALSE}
sort(factor(x1, levels = month_levels))
```

```{r factor-basics-7-check}
grade_this_code()
```
...
```{r factor-basics-8-solution, exercise.reveal_solution = FALSE}
factor(x2, levels = month_levels)
```

```{r factor-basics-8-check}
grade_this_code()
```
...
```{r factor-basics-9-solution, exercise.reveal_solution = FALSE}
tryCatch(
...
grade_this({
  ...
  error_check = 'All values of `x` must appear in `levels` or `na`\\nℹ Missing level: "Jam"'
...
```{r factor-basics-9-check}
grade_this({
  if (TRUE) {
    fail("Try again. Your code should be producing an error!")
  }
})
```
...
Your code should have produced an error: 
...
This is a thorough error message. `"Jam"` is missing from the `levels`, which we defined in the `month_levels` variable.
...
```{r factor-basics-10-solution, exercise.reveal_solution = FALSE}
factor(x1)
```

```{r factor-basics-10-check}
grade_this_code()
```
...
```{r factor-basics-11-solution, exercise.reveal_solution = FALSE}
fct(x1)
```

```{r factor-basics-11-check}
grade_this_code()
```
...
```{r factor-basics-12-solution, exercise.reveal_solution = FALSE}
levels(fct(x1))
```

```{r factor-basics-12-check}
grade_this_code()
```
...
The next exercises will be focusing on the variable `csv`. Click "Submit Answer" to look at `csv`.

```{r factor-basics-13, exercise = TRUE}
csv
```

```{r factor-basics-13-solution, exercise.reveal_solution = FALSE}
csv
```

```{r factor-basics-13-check}
grade_this_code()
```
...
```{r factor-basics-14-hint, eval = FALSE}
read_csv(...)
```

```{r factor-basics-14-solution, exercise.reveal_solution = FALSE}
read_csv(csv)
```

```{r factor-basics-14-check}
grade_this_code()
```
...
```{r factor-basics-15-solution, exercise.reveal_solution = FALSE}
read_csv(csv, col_types = "cc")
```

```{r factor-basics-15-check}
grade_this_code()
```
...
```{r factor-basics-16-solution, exercise.reveal_solution = FALSE}
read_csv(csv, col_types = cols(month = "c"))
```

```{r factor-basics-16-check}
grade_this_code()
```
...
```{r factor-basics-17-solution, exercise.reveal_solution = FALSE}
read_csv(csv, col_types = cols(month = "f"))
```

```{r factor-basics-17-check}
grade_this_code()
```
...
```{r factor-basics-18-solution, exercise.reveal_solution = FALSE}
read_csv(csv, col_types = cols(month = "f")) |> 
  count(month)
```

```{r factor-basics-18-check}
grade_this_code()
```
...
```{r factor-basics-19-solution, exercise.reveal_solution = FALSE}
read_csv(csv, 
         col_types = cols(month = col_factor(month_levels))) |>  
  count(month)
```

```{r factor-basics-19-check}
grade_this_code()
```
...
Type `gss_cat` and click "Submit Answer."
...
```{r general-social-surve-1-solution, exercise.reveal_solution = FALSE}
gss_cat
```

```{r general-social-surve-1-check}
grade_this_code()
```
...
question_text(NULL,
	answer("A sample of categorical variables from the General Social survey", correct = TRUE),
	allow_retry = TRUE,
...
```{r general-social-surve-3-solution, exercise.reveal_solution = FALSE}
gss_cat |>
  count(race)
```

```{r general-social-surve-3-check}
grade_this_code()
```
...
plot1 <- gss_cat |>
  ...
  geom_point() +
  labs(
    title = "TV Watching and Religious Affiliation",
...
```{r modifying-factor-ord-1-solution, exercise.reveal_solution = FALSE}
glimpse(gss_cat)
```

```{r modifying-factor-ord-1-check}
grade_this_code()
```
...
```{r modifying-factor-ord-2-solution, exercise.reveal_solution = FALSE}
# option 1 ----
gss_cat |>
  summarize(n = n())
...
```{r modifying-factor-ord-2-check}
grade_this_code()
```
...
```{r modifying-factor-ord-3-solution, exercise.reveal_solution = FALSE}
# option 1 ----
gss_cat |>
  summarize(
    n = n(),
...
```{r modifying-factor-ord-3-check}
grade_this_code()
```
...
```{r modifying-factor-ord-4-solution, exercise.reveal_solution = FALSE}
# option 1 ----
gss_cat |>
  summarize(
    n = n(),
...
```{r modifying-factor-ord-4-check}
grade_this_code()
```
...
```{r modifying-factor-ord-5-solution, exercise.reveal_solution = FALSE}
# option 1 ----
gss_cat |>
  summarize(
    n = n(),
...
```{r modifying-factor-ord-5-check}
grade_this_code()
```
...
```{r modifying-factor-ord-6-solution, exercise.reveal_solution = FALSE}
# option 1 ----
gss_cat |>
  summarize(
    n = n(),
...
```{r modifying-factor-ord-6-check}
grade_this_code()
```
...
```{r modifying-factor-ord-7-solution, exercise.reveal_solution = FALSE}
# option 1 ----
gss_cat |>
  summarize(
    n = n(),
...
```{r modifying-factor-ord-7-check}
grade_this_code()
```
...
```{r modifying-factor-ord-8-solution, exercise.reveal_solution = FALSE}
# option 1 ----
gss_cat |>
  summarize(
    n = n(),
...
```{r modifying-factor-ord-8-check}
grade_this_code()
```

### 

Reordering religion makes it much easier to see that people in the `“Don’t know”` category watch much more TV, and Hinduism & Other Eastern religions watch much less TV.
...
```{r modifying-factor-ord-9-solution, exercise.reveal_solution = FALSE}
# option 1 ----
gss_cat |>
  summarize(
    n = n(),
...
```{r modifying-factor-ord-9-check}
grade_this_code()
```
### 
...
... +
  labs(
    ... = "TV Watching and Religious Affiliation",
    subtitle = ...,
    x = ...,
...
```{r modifying-factor-ord-10-solution, exercise.reveal_solution = FALSE}
# option 1 ----
gss_cat |>
  summarize(
    n = n(),
...
```{r modifying-factor-ord-10-check}
grade_this_code()
```
...
```{r modifying-factor-ord-11-solution, exercise.reveal_solution = FALSE}
gss_cat |>
  filter(!is.na(age))
```

```{r modifying-factor-ord-11-check}
grade_this_code()
```
...
```{r modifying-factor-ord-12-solution, exercise.reveal_solution = FALSE}
gss_cat |>
  filter(!is.na(age)) |>
  count(age, marital)
```

```{r modifying-factor-ord-12-check}
grade_this_code()
```
...
```{r modifying-factor-ord-13-solution, exercise.reveal_solution = FALSE}
gss_cat |>
  filter(!is.na(age)) |>
  count(age, marital) |>
  mutate(prop = n/sum(n))
...
```{r modifying-factor-ord-13-check}
grade_this_code()
```
...
```{r modifying-factor-ord-14-solution, exercise.reveal_solution = FALSE}
gss_cat |>
  filter(!is.na(age)) |>
  count(age, marital) |>
  mutate(
...
```{r modifying-factor-ord-14-check}
grade_this_code()
```
...
```{r modifying-factor-ord-15-solution, exercise.reveal_solution = FALSE}
gss_cat |>
  filter(!is.na(age)) |>
  count(age, marital) |>
  mutate(
...
```{r modifying-factor-ord-15-check}
grade_this_code()
```
...
```{r modifying-factor-ord-16-solution, exercise.reveal_solution = FALSE}
gss_cat |>
  filter(!is.na(age)) |>
  count(age, marital) |>
  mutate(
...
```{r modifying-factor-ord-16-check}
grade_this_code()
```
...
```{r modifying-factor-ord-17-solution, exercise.reveal_solution = FALSE}
gss_cat |>
  filter(!is.na(age)) |>
  count(age, marital) |>
  mutate(
...
```{r modifying-factor-ord-17-check}
grade_this_code()
```

### Exercise 18

Finish the pipe with `labs()` giving an appropriate x axis, y axis and color legend titles of "Age", "Proportion", and "Marital", respectively.
...
```{r modifying-factor-ord-18-solution, exercise.reveal_solution = FALSE}
gss_cat |>
  filter(!is.na(age)) |>
  count(age, marital) |>
  mutate(
  ...
  geom_line(linewidth = 1) +
  scale_color_brewer(palette = "Set1") +
  labs(x = "Age", y = "Proportion", color = "Marital")
```

```{r modifying-factor-ord-18-check}
grade_this_code()
```
...
```{r modifying-factor-ord-19-solution, exercise.reveal_solution = FALSE}
gss_cat |>
  filter(!is.na(age)) |>
  count(age, marital) |>
  mutate(
...
```{r modifying-factor-ord-19-check}
grade_this_code()
```
...
```{r modifying-factor-lev-1-solution, exercise.reveal_solution = FALSE}
gss_cat |> 
  count(partyid)
```

```{r modifying-factor-lev-1-check}
grade_this_code()
```
...
```{r modifying-factor-lev-2-solution, exercise.reveal_solution = FALSE}
gss_cat |>
  mutate(
...
```{r modifying-factor-lev-2-check}
grade_this_code()
```
...
```{r modifying-factor-lev-3-solution, exercise.reveal_solution = FALSE}
gss_cat |>
  mutate(
    partyid = fct_recode(partyid,
      "Republican, strong"    = "Strong republican",
...
```{r modifying-factor-lev-3-check}
grade_this_code()
```
### 
...
```{r modifying-factor-lev-4-solution, exercise.reveal_solution = FALSE}
gss_cat |>
  mutate(
    partyid = fct_recode(partyid,
      "Republican, strong"    = "Strong republican",
...
```{r modifying-factor-lev-4-check}
grade_this_code()
```
...
```{r modifying-factor-lev-5-solution, exercise.reveal_solution = FALSE}
gss_cat |>
  mutate(
    partyid = fct_recode(partyid,
      "Republican, strong"    = "Strong republican",
...
```{r modifying-factor-lev-5-check}
grade_this_code()
```
...
```{r modifying-factor-lev-6-solution, exercise.reveal_solution = FALSE}
gss_cat |>
  mutate(
...
```{r modifying-factor-lev-6-check}
grade_this_code()
```
...
```{r modifying-factor-lev-7-solution, exercise.reveal_solution = FALSE}
gss_cat |>
  mutate(relig = fct_lump_lowfreq(relig))
```

```{r modifying-factor-lev-7-check}
grade_this_code()
```
...
```{r modifying-factor-lev-8-solution, exercise.reveal_solution = FALSE}
gss_cat |>
  mutate(relig = fct_lump_lowfreq(relig)) |>
  count(relig)
```

```{r modifying-factor-lev-8-check}
grade_this_code()
```
...
```{r modifying-factor-lev-9-solution, exercise.reveal_solution = FALSE}
gss_cat |>
  mutate(relig = fct_lump_n(relig, n = 10))
```

```{r modifying-factor-lev-9-check}
grade_this_code()
```
...
```{r modifying-factor-lev-10-solution, exercise.reveal_solution = FALSE}
gss_cat |>
  mutate(relig = fct_lump_n(relig, n = 10)) |>
  count(relig, sort = TRUE)
```

```{r modifying-factor-lev-10-check}
grade_this_code()
```
...
```{r ordered-factors-1-solution, exercise.reveal_solution = FALSE}
ordered(c("a", "b", "c"))
```

```{r ordered-factors-1-check}
grade_this_code()
```
