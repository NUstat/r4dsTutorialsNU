...
```{r introduction-4}
question_text(NULL,
	answer("Includes measurements for penguin species, island in Palmer Archipelago, size (flipper length, body mass, bill dimensions), and sex. This is a subset of penguins_raw.", correct = TRUE),
	allow_retry = TRUE,
...
```{r first-steps-3a, exercise = TRUE}

```

```{r first-steps-3a-hint-1, eval = FALSE}
ggplot(data = ___, mapping = aes(x = ___, y = ___)) +
  geom_*()
```

```{r first-steps-3a-solution, exercise.reveal_solution = FALSE}
ggplot(
  data = penguins,
...
Copy your previous code chunk only this time pipe your data into the `drop_na()` function and then pipe the result into your plot.


`data |> function() |> ggplot(aes(...)) + geom_*`

```{r first-steps-3b, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r first-steps-3b-hint-1, eval = FALSE}
penguins |>
  ...
```

```{r first-steps-3b-solution, exercise.reveal_solution = FALSE}
penguins |> 
  drop_na() |> 
  ggplot(
  data = penguins,
...
Now start by copying your previous code chunk and adding `color = species` within `aes()` and adding the `geom_smooth()` layer using `+` with `method` equal to `"lm"` within the call to `geom_smooth()`. 
...
```{r first-steps-4-hint-2, eval = FALSE}
penguins |>
  drop_na() |>
  ggplot(mapping = aes(x = flipper_length_mm, 
                       y = body_mass_g, 
...
```{r first-steps-5a, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r first-steps-5a-hint-1, eval = FALSE}
penguins |>
  drop_na() |>
  ggplot(mapping = aes(x = flipper_length_mm, 
                       y = body_mass_g)) +
...
```{r first-steps-5a-solution, exercise.reveal_solution = FALSE}
penguins |>
  drop_na() |>
  ggplot(mapping = aes(x = flipper_length_mm, 
                       y = body_mass_g)) +
...
```{r first-steps-5a-check}
grade_this_code()
```

### 

Notice the legend looks a little different than our final goal. We forgot one minor thing which is capitalizing the legend, we can do so by typing a comma and setting `color` equal to "Species" in quotes.

```{r first-steps-5b, exercise = TRUE}
...
```{r first-steps-5b-solution, exercise.reveal_solution = FALSE}
penguins |>
  drop_na() |>
  ggplot(mapping = aes(x = flipper_length_mm, 
                       y = body_mass_g)) +
...
```{r first-steps-6a, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r first-steps-6a-hint-1, eval = FALSE}
penguins |>
  drop_na() |>
  ggplot(mapping = aes(x = flipper_length_mm, 
                       y = body_mass_g)) +
...
```{r first-steps-6a-solution, exercise.reveal_solution = FALSE}
penguins |>
  drop_na() |>
  ggplot(mapping = aes(x = flipper_length_mm, 
                       y = body_mass_g)) +
...
Finally, improve the color palette to be colorblind safe by adding the `scale_color_colorblind()` function from the `ggthemes` package.

```{r first-steps-6b, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r first-steps-6b-solution, exercise.reveal_solution = FALSE}
penguins |>
  drop_na() |>
  ggplot(mapping = aes(x = flipper_length_mm, 
                       y = body_mass_g)) +
...
```{r vis-dist-3a, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r vis-dist-3a-hint-1, eval = FALSE}
ggplot(penguins, aes(x = ..., fill = ...)) +
  geom_bar()
```


```{r vis-dist-3a-solution, exercise.reveal_solution = FALSE}
...
```{r vis-dist-3b, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r vis-dist-3b-hint-1, eval = FALSE}
ggplot(penguins, aes(x = island, fill = species)) +
  geom_bar(position = ...)
```


```{r vis-dist-3b-solution, exercise.reveal_solution = FALSE}
ggplot(penguins, aes(x = island, fill = species)) +
...
```{r vis-dist-4a, exercise = TRUE}

```

```{r vis-dist-4a-hint-1, eval = FALSE}
ggplot(penguins, ...) +
  geom_*()
```


```{r vis-dist-4a-solution, exercise.reveal_solution = FALSE}
...
```{r vis-dist-4b, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r vis-dist-4b-hint-1, eval = FALSE}
... +
  geom_histogram(bins = ...)
```

```{r vis-dist-4b-solution, exercise.reveal_solution = FALSE}
ggplot(penguins, aes(x = body_mass_g)) +
...
```{r vis-dist-4c, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r vis-dist-4c-hint-1, eval = FALSE}
... +
  geom_histogram(binwidth = ...)
```

```{r vis-dist-4c-solution, exercise.reveal_solution = FALSE}
ggplot(penguins, aes(x = body_mass_g)) +
...
```{r vis-dist-6a, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r vis-dist-6a-hint-1, eval = FALSE}
ggplot(penguins, aes(x = body_mass_g, color = ..., fill = ...)) +
  geom_density()
```

```{r vis-dist-6a-solution, exercise.reveal_solution = FALSE}
...
```{r vis-dist-6b, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r vis-dist-6b-hint-1, eval = FALSE}
... +
  geom_density(alpha = ...)
```

```{r vis-dist-6b-solution, exercise.reveal_solution = FALSE}
ggplot(penguins, aes(x = body_mass_g, color = species, fill = species)) +
...
Copy the previous plot, then add `ggtitle()` to define the title as "Body Mass Distribution of Palmer Penguins by Species: A Boxplot Analysis"; followed by `xlab()` to define the x label as `Species`; followed by the `ylab()` to define the y label as"Body Mass(g)"
...
ggplot(penguins, aes(x = species, y = body_mass_g)) +
  geom_boxplot() +
  ggtitle("Body Mass Distribution of Palmer Penguins by Species: A Boxplot Analysis") +
  xlab("Species") +
  ylab("Body Mass(g)")
```
...
```{r vis-relation-4a, exercise = TRUE}
___ |> 
  ___ |> 
...
```{r vis-relation-4a-solution, exercise.reveal_solution = FALSE}
penguins |>
  drop_na() |> 
  ggplot(aes(x = flipper_length_mm, 
             y = body_mass_g)) +
...
```{r vis-relation-4b, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r vis-relation-4b-hint-1, eval = FALSE}
... +
  facet_wrap(...)
```

```{r vis-relation-4b-solution, exercise.reveal_solution = FALSE}
penguins |>
  drop_na() |> 
  ggplot(aes(x = flipper_length_mm, 
             y = body_mass_g)) +
...
```{r vis-relation-4c, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r vis-relation-4c-hint-1, eval = FALSE}
... +
  facet_wrap(~island, ncol = ..., scales = ...)
```

```{r vis-relation-4c-solution, exercise.reveal_solution = FALSE}
penguins |>
  drop_na() |> 
  ggplot(aes(x = flipper_length_mm, 
             y = body_mass_g)) +
...
Add `aes()` within `ggplot()` and specify `x` to `flipper_length_mm` and `y` to `body_mass_g`. Add the appropriate `geom` layer for a scatterplot.

```{r saving-plots-1a, exercise = TRUE}

```

```{r saving-plots-1a-hint-1, eval = FALSE}
penguins |> 
  ...() |> 
  ggplot(aes(...)) +
  geom_()
...
```{r saving-plots-1a-solution, exercise.reveal_solution = FALSE}
penguins |> 
  drop_na() |> 
  ggplot(aes(x = flipper_length_mm, 
             y = body_mass_g)) +
...
```{r saving-plots-1b, exercise = TRUE}

```

```{r saving-plots-1b-hint-1, eval = FALSE}
ggsave(filename = ...)
```

```{r saving-plots-1b-solution, exercise.reveal_solution = FALSE}
ggsave(filename = "penguin_plot.png")
```

```{r saving-plots-1b-check}
grade_this_code()
```
...
```{r saving-plots-2a, exercise = TRUE}
penguins |>   
  drop_na() |>
  ggplot(aes(x = flipper_length_mm, 
             y = body_mass_g)) +
...
```{r saving-plots-2a-hint-1, eval = FALSE}
... <- penguins |>   
  drop_na() |>
  ggplot(aes(x = flipper_length_mm, 
             y = body_mass_g)) +
...
```{r saving-plots-2a-solution, exercise.reveal_solution = FALSE}
plot_geom <- penguins |>   
  drop_na() |>
  ggplot(aes(x = flipper_length_mm, 
             y = body_mass_g)) +
...
```{r saving-plots-2a-check}
grade_this_code()
```

### 

This time within `ggsave()`, set `filename` to `"penguin_plot.png"` and then set `plot` to `plot_geom` which was our plot that we made previously.

```{r saving-plots-2b, exercise = TRUE}

```

```{r saving-plots-2b-hint-1, eval = FALSE}
ggsave(filename = ..., plot = ...)
```

```{r saving-plots-2b-solution, exercise.reveal_solution = FALSE}
ggsave(filename = "penguin_plot.png", plot = plot_geom)
```

```{r saving-plots-2b-check}
grade_this_code()
```
### 
