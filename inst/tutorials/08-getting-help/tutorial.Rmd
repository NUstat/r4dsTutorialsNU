...
If you get stuck, start with Google. Typically adding “R” or "R tidyverse" to a query is enough to restrict it to relevant results. If the search result isn’t useful, it often means that there aren’t any R-specific results available.
...
Sometimes, you may need to use a double question mark --- `??function_name` --- if the function is part of a package you don't have loaded. RStudio will prompt you to try `??` if `?` doesn't work. 
...
Open a new R Script file. This R Script should be in your current R project folder. Save your R Script file and name it `example_reprex.R`.
...
`ggplot(mtcars, aes(x = cyl, y = mpga)) +`
<br> &nbsp; `geom_col()`
...
As you can see, there is an error about us not loading **tidyverse** and as such not having the `ggplot()` function available in our R script.
...
Let's fix that. Load **tidyverse** using the `library()` command at the top of `example_reprex.R`. 

Then run the code and **paste the first line of the error message below**.

```{r making-a-reprex-1}
question_text(NULL,
  answer_fn(function(value) {
    if (str_remove_all(str_to_lower(value), " ") %in%
      str_to_lower(c("Errorin`geom_col()`:", "Errorin`geom_col()`"))) {
      return(mark_as(TRUE))
    }
    return(mark_as(FALSE))
  }),
...
Run `data()` in your console and copy and paste the name of the first dataset that appears.

```{r making-a-reprex-2}
question_text(NULL,
	answer("AirPassengers", correct = TRUE),
	allow_retry = TRUE,
	try_again_button = "Edit Answer",
	incorrect = NULL,
	rows = 3)
...
Now let's create a reprex. Select all the text in your `example_reprex.R` Script file. Then go to "Addins" at the top of RStudio. 
...
Copy all of the code in `example_reprex.R` onto your clipboard then run `reprex()` in the Console.
