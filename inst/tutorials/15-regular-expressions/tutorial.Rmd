...
Run `stringr::fruit` in your console. What is the name of the first fruit?
...
Type `str_view(fruit, "berry")`. Hit "Submit Answer"
...
`str_view()` will show only the elements of the string vector that match, surrounding each match with <>, and, where possible, highlighting the match in blue or by surrounding the match with `< >`.
...
<!-- Modify exercises to show non-matches too -->
...
We are matching all the words which feature the pattern of any vowel, followed by an "x", followed by any vowel.
...
Load the **babynames** package with `library()` below. Don't forget to hit "Submit Answer"
...
The definition of the `prop` variable is subtle, so makes sure you pay special attention to it.
...
Run `str_detect()` with the `string` argument equal to `c("a", "b", "c")` and the `pattern` argument equal to  `"[aeiou]"`.
...
str_detect(string = c("a", "b", "c"), pattern = "[aeiou]")
```
...
Pipe `babynames` to `filter()`, using `str_detect(name, "x")` as the argument to `filter()`.
...
Note that `name` is one of the variables in `babynames`. This pipe reduces the almost 2 million rows in `babynames` to just over 16,300 or so rows in which `name` contains the letter `x`.
...
This gives us the proportion of names that contain an "x". If you wanted the proportion of babies with a name containing an "x", then you would need to perform a weighted mean.
...
Run `str_count()` with two arguments with `c("apple", "banana", "pear")` as your string and the letter `"p"` as the pattern.
...
Run `str_count()` on the string `"abababa"` and matching the pattern `"aba"`.
...
For example, in "abababa", how many times will the pattern "aba" match? Regular expressions say 2, not 3.

### Exercise 13

To better see this, apply `str_view()` to the string `"abababa"` while matching the pattern and `"aba"`.
...
Run `str_replace_all()` on with `string` equal to `c("apple", "pear", "banana")`, `pattern` equal to  `"[aeiou]"`, and `replacement` equal to `"-"`.
...
Run `str_remove_all()` on `c("apple", "pear", "banana")` and `"[aeiou]"`.
...
Click "Submit Answer" to examine `people` tibble/data frame.
...
Click "Submit Answer" to see how this works.
...
Click "Submit Answer" to demonstrate.
...
Probably better is to use the raw strings you learned about previously. That lets you avoid one layer of escaping.
...
`str_view()` highlights the single backslash which is part of the `x` variable. But, to match that single backslash, we need a raw string pattern with two backslashes.
...
Regular expressions require care and attention. That last example, which uses `".[*]c"`, means any character (the `.`), followed by a `*` (captured as a character class), followed by a "c". Only `"a*c"` matches this pattern.
...
Run `str_view()` twice, both times with `fruit` as the first argument. In one, `"apple"` is the pattern. In the second, `"^apple$"` is the pattern.
...
Add `str_view(x, "sum")` to this exercise block
...
The previous example helps you understand what happens when you replace a standalone anchor. 

Run `str_replace_all()` on the string `"abc"` and the patterns `c("$", "^", "\\b")` while using  `"--"` as the replacement.
...
If you want to learn more, a good place to start is [`vignette("regular-expressions", package = "stringr")`](https://stringr.tidyverse.org/articles/regular-expressions.html): it documents the full set of syntax supported by the [**stringr**](https://stringr.tidyverse.org/index.html) package. 

Another useful reference is `https://www.regular-expressions.info/`. It’s not R specific, but you can use it to learn about the most advanced features of regexes and how they work under the hood.
