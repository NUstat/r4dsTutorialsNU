...
A scatterplot like this is the most common way to look at data. If there is a lot of overlap among the data points, you may want to use [`geom_jitter()`](https://ggplot2.tidyverse.org/reference/geom_jitter.html) to separate the points.
...
ggplot(mpg, aes(x = displ, y = hwy, shape = ...)) +
  geom_point()
```
...
ggplot(mpg, aes(x = displ, y = hwy, size = ...)) +
  geom_point()
```
...
ggplot(mpg, aes(x = displ, y = hwy, alpha = ...)) +
  geom_point()
```
...
ggplot(mpg, aes(x = displ, y = hwy, linetype = ...)) + 
  geom_smooth(method = 'loess', formula = 'y ~ x', se = FALSE)
```
...
ggplot(mpg, aes(x = displ, y = hwy, linetype = drv, color = ...)) + 
  ...() +
  geom_smooth(method = 'loess', formula = 'y ~ x', se = FALSE)
```
...
Set `linetype = drv` in `aes()` from within `geom_smooth()`. 
...
Make a new plot by hitting "Submit Anwser".
...
grade_this_code(
  correct = "Complete!", 
  incorrect = "Try again! Must have changed something. Hit Start Over button."
)
```
### 
...
The [**ggridges** package](https://wilkelab.org/ggridges) is useful for making ridgeline plots, which can be useful for visualizing the density of a numerical variable for different levels of a categorical variable. Hit "Submit Answer" to see an example.
...
Start with our basic scatterplot of `displ` versus `hwy` using the `mpg` tibble/dataset.
...
Setting `group`, is used to override the default grouping of `cut`. If we did not set this argument it would instead set each proportion to 1 (or 100% ) because the proportion of Fair in Fair is 1 (or 100%), etc.

### Exercise 6

You might want to draw greater attention to the statistical transformation in your code. For example, you might use stat_summary(), which summarizes the y values for each unique x value, to draw attention to the summary that you’re computing. Hit "Submit Answer".
...
`position = "fill"` works like stacking, but makes each set of stacked bars the same height. Use the code from the exercise, but remove the alpha argument and replace `"identity"` with `"fill"`.
...
Hit "Submit Answer" to create an object with data for New Zealand and then create a plot.
