...
quiz(
  caption = "",
  question_text("What function provides an easy way to see all variables and variable types which is particularly useful for large datasets?",
          answer_fn(function(value){
...
```{r rows-2b, exercise = TRUE}

```

```{r rows-2b-hint-1, eval = FALSE}
flights |> 
  filter(...)
```

```{r rows-2b-solution, exercise.reveal_solution = FALSE}
flights |>
...
```{r rows-2c, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r rows-2c-hint-1, eval = FALSE}
flights |>
  filter(dep_delay > 120) |> 
  ...
```

```{r rows-2c-solution, exercise.reveal_solution = FALSE}
flights |>
  filter(dep_delay > 120) |> 
  nrow()
```
...
``` {r columns-1-test, include = FALSE}
flights |>
...
``` {r columns-2-test, include = FALSE}
flights |>
  mutate(gain = dep_delay - arr_delay,
...
``` {r columns-3-test, include = FALSE}
flights |>
  mutate(gain = dep_delay - arr_delay,
         speed = distance / air_time * 60,
         .before = 1)
...
``` {r columns-4-test, include = FALSE}
flights |> 
  mutate(
    gain = dep_delay - arr_delay,
    speed = distance / air_time * 60,
...
``` {r columns-5-test, include = FALSE}
flights |> 
  mutate(
    gain = dep_delay - arr_delay,
    speed = distance / air_time * 60,
...
``` {r columns-6-test, include = FALSE}
flights |>
...
``` {r columns-7-test, include = FALSE}
flights |>
  select(year:day)
```
...
``` {r columns-8-test, include = FALSE}
flights |>
  select(! year:day)
```
...
``` {r columns-9-test, include = FALSE}
flights |>
...
``` {r columns-10-test, include = FALSE}
flights |>
...
``` {r columns-11-test, include = FALSE}
flights |>
...
``` {r columns-12-test, include = FALSE}
flights |>
...
``` {r the_pipe-1-test, include = FALSE}
flights |>
  filter(dest == "IAH")
```
...
``` {r the_pipe-2-test, include = FALSE}
flights |> 
  filter(dest == "IAH") |> 
...
``` {r the_pipe-3-test, include = FALSE}
flights |> 
  filter(dest == "IAH") |> 
...
``` {r the_pipe-4-test, include = FALSE}
flights |> 
  filter(dest == "IAH") |> 
...
``` {r the_pipe-5-test, include = FALSE}
flights |> 
  filter(dest == "IAH") |> 
  mutate(speed = distance / air_time * 60, .before = 1) |>
...
``` {r the_pipe-6-test, include = FALSE}
flights |> 
  filter(dest == "IAH") |> 
  mutate(speed = distance / air_time * 60, .before = 1) |>
  select(year:day, dep_time, carrier, flight, speed) |>
...
```{r groups-10-test, include = FALSE}
flights |> 
...
```{r groups-11-test, include = FALSE}
flights |> 
