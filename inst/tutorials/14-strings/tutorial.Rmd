...
Now create a new variable `x` and set it to the letter `n` using `<-` in a string format using double quotes. Then on a new line call x.
...
To illustrate this problem, let's create a variable called `tricky` and set it to 
...
Click "Submit Answer" to complete the assignment.
...
After running the code, we should expect to see 
...
Let's copy the previous code and modify tricky to be 
...
After modifying the string, run `str_view()` with `tricky` as the argument.
...
In most cases, it printed the exact same thing we typed except for the $\mu$ and smiley face emoji. In those cases, it recognized the unicode escapes (`\u` and `\U`) which is a why to print non-english characters that work on all systems. If you want to check out a complete list of other special characters, check out in [Quotes](https://rdrr.io/r/base/Quotes.html).
...
Compared to just printing the strings in console, we actually see the new line for the first string, we see a the tab in the second string which shows as `{\t}`, and the others show as expected. So `str_view` can take in special characters and display them as expected. 
...
`str_c()` is a function in the `stringr` package in R that combines multiple character vectors into a single character vector. It is similar to the `paste0()` function, but it uses `tidyverse` recycling and NA rules.
...
Recall, a tibble is a new form of a data frame in R that is part of the tidyverse library. Tibbles print the data in a more efficient format than a data frame, showing the values of the columns, their datatype, and the size of the dataset. They also prevent the printing of unnecessarily long output. 
...
After modifying the tibble, we still have a  column called `name`. We used the `mutate()` function from the [**dplyr**](https://dplyr.tidyverse.org/) package to create a new column called `greeting`. The `str_c()` function is used to concatenate the string `"Hi "` with the values in the `name` column along with `"!"`. 
...
Modify the code from the last exercise to set `greeting` to `str_glue("Hi {name}!")`.
...
What if you want commas after every letter. Copy the previous code and set place `","` as the second argument.
...
The second argument which was set to a comma, is known as the `collapse` argument: a string to enter between each piece. It defaults to `""`, no space.

### Exercise 9

Let's fix the grammar of the string we got in previous exercise, copy the previous code and add `last = ", and "`. Also, modify the collapse to have a space after the comma: `", "`
...
Let's now use `str_flatten()` with `summarize()`. Run `df1` to have a quick look at the data frame.
...
Start a pipe with `df1` to `summarize()` and within `summarize()`, type the column `fruit` and set it to `str_flatten(fruit, ", ")`. 
...
Click "Submit Answer", which will create and print a new tibble using `tibble()`, with a single column named `x` that is set to  `c("a,b,c", "d,e", "f")`.

```{r extracting-data-from-strings-1, exercise = TRUE}
df3 <- tibble(x = c("a,b,c", "d,e", "f"))
...
Copy the previous code and now pipe `df3` to `separate_longer_delim()`. Within the function, type the column `x` and set `delim` equal to `","`.
...
Sometimes distinct numbers within a string are not separated by commas or spaces. In such cases, how should we properly split the string to separate these numbers? 

To demonstrate the issue, click "Submit Answer" to create and print a new `tibble()` named `df4`, with one column named `x` that is set to the vector `c("1211", "131", "21")`.

```{r extracting-data-from-strings-3, exercise = TRUE}
df4 <- tibble(x = c("1211", "131", "21"))
...
Copy the previous code, but now pipe `df4` to `separate_longer_position()`, within the function, type the column `x` and then set the `width` to 1.
...
Click "Submit Answer" to create and print a new `tibble()` named `df5`, with one column named `x` that is set to the vector `c("a10.1.2022", "b10.2.2011", "e15.1.2015")`.


```{r extracting-data-from-strings-5, exercise = TRUE}
df5 <- tibble(x = c("a10.1.2022", "b10.2.2011", "e15.1.2015"))
...
In the `df5` data set, x is made up of a code, an edition number, and a year, separated by `"."`. To use `separate_wider_delim()`, we supply the delimiter and the names in two arguments.

### Exercise 6

Copy the previous code, but now pipe `df5` into `separate_wider_delim()`, within the function, set `cols` to `x`, then set the `delim` to `"."` and finally set the `names` to `c("code", "edition", "year")`.
...
Click "Submit Answer" to create and print a new `tibble()` named `df6`, with one column named `x` that is set to the vector `c("202215TX", "202122LA", "202325CA")`.

```{r extracting-data-from-strings-8, exercise = TRUE}
df6 <- tibble(x = c("202215TX", "202122LA", "202325CA"))
...
Copy the previous code, but now pipe `df6` into `separate_wider_position()`, within the function, set `cols` to  `x`, then set `widths` to  `c(year = 4, age = 2, state = 2)`.
...
Click "Submit Answer" to create and print a new `tibble()` named `df7`, with one column named `x` that is set to the vector `c("1-1-1", "1-1-2", "1-3", "1-3-2", "1")`.


```{r extracting-data-from-strings-10, exercise = TRUE}
df7 <- tibble(x = c("1-1-1", "1-1-2", "1-3", "1-3-2", "1"))
...
Click "Run Code" to see the error. Notice the error gives us some suggestions on how you might proceed. 

Let's start to debug the problem.

Within `separate_wider_delim()` add the argument `too_few` and set it to be `"debug"`. Assign the result of this pipe line to `df7_debug`. Also, add a new line and call `df7_debug` to print the result.
...
Consider a new `tibble()` named `df8`, with one column named `x` that is set to the vector `c("1-1-1", "1-1-2", "1-3-5-6", "1-3-2", "1-3-5-7-9")`.

Click "Run Code" to see the error this case produces. The error message suggests to use `too_many = "debug"` or `too_many = "drop/merge"`. We will use `"debug"` to see what is being kept and what's being left and we will explore `"drop/merge"` later.

Within `separate_wider_delim()` add the argument `too_many` to be `"debug"`.

```{r extracting-data-from-strings-13, exercise = TRUE}
df8 <- tibble(x = c("1-1-1", "1-1-2", "1-3-5-6", "1-3-2", "1-3-5-7-9"))
...
How do you find length of a string? That's where `str_length()` comes into play. `str_length()` tells you the number of characters in a string.
...
In addition to `"hello world"` in `str_length()`, add two more values which are `"programming"` and `NA`. Make sure to put these three string in vector like `c(...,...,...)`. See *Hint* for help.
...
We have five columns (`year`, `sex`, `name`, `n`, `prop`). The `n` means the count of the name and the `prop` is the proportion n divided by total number of applicants in that year, which means proportions are of people of that sex with that name born in that year.
...
We see that there are a total count of 1,924,665 names used. Let's make the function more complex to find the distribution of lengths of baby names.
...
If you didn't know, in Python, indexing starts from zero, whereas in R, indexing starts from one, resulting in a slight variation in accessing elements and slicing sequences.
