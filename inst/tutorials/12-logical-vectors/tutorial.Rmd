...
<!-- Modification of r4ds.tutorials author: David Kane, Zayan Farooq, and Rajarshi Mandal -->
...
gradethis_setup(exercise.error.check.code = gradethis_error_checker)
...
```{r comparisons-1-solution, exercise.reveal_solution = FALSE}
library(tidyverse)
```

```{r comparisons-1-check}
grade_this_code()
```
...
```{r comparisons-2-solution, exercise.reveal_solution = FALSE}
library(nycflights13)
```

```{r comparisons-2-check}
grade_this_code()
```
...
Type `flights` and hit "Submit Answer".

```{r comparisons-3, exercise = TRUE}
```

```{r comparisons-3-solution, exercise.reveal_solution = FALSE}
flights
```

```{r comparisons-3-check}
grade_this_code()
```
...
```{r comparisons-4-solution, exercise.reveal_solution = FALSE}
flights |>
  filter(dep_time > 600)
```

```{r comparisons-4-check}
grade_this_code()
```
...
```{r comparisons-5-solution, exercise.reveal_solution = FALSE}
flights |>
  filter(dep_time > 600 & dep_time < 2000)
```

```{r comparisons-5-check}
grade_this_code()
```
...
```{r comparisons-6-solution, exercise.reveal_solution = FALSE}
flights |>
  filter(dep_time > 600 & dep_time < 2000 & abs(arr_delay) < 20)
```

```{r comparisons-6-check}
grade_this_code()
```
...
```{r comparisons-7-solution, exercise.reveal_solution = FALSE}
flights |>
  mutate(daytime = dep_time > 600 & dep_time < 2000)
```

```{r comparisons-7-check}
grade_this_code()
```
...
```{r comparisons-8-solution, exercise.reveal_solution = FALSE}
flights |>
  mutate(
    daytime = dep_time > 600 & dep_time < 2000,
...
```{r comparisons-9-solution, exercise.reveal_solution = FALSE}
flights |>
  mutate(
    daytime = dep_time > 600 & dep_time < 2000,
    approx_ontime = abs(arr_delay) < 20,
...
```{r comparisons-10-solution, exercise.reveal_solution = FALSE}
flights |>
  mutate(
    daytime = dep_time > 600 & dep_time < 2000,
    approx_ontime = abs(arr_delay) < 20,
...
Set `x` to `c(1 / 49 * 49, sqrt(2) ^ 2)` using `<-`. On the next line type `x` to print the result.
...
```{r comparisons-11-solution, exercise.reveal_solution = FALSE}
x <- c(1 / 49 * 49, sqrt(2)^2)
x
```

```{r comparisons-11-check}
grade_this_code()
```
...
```{r comparisons-12-solution, exercise.reveal_solution = FALSE}
x == c(1, 2)
```

```{r comparisons-12-check}
grade_this_code()
```

### 

Beware of using `==` with numbers. The expected output is `TRUE`.
...
We can see the exact values by calling `print()` with the digits argument. Run `print(x, digits = 16)`. This will run the values of `x` with 16 digits.
...
```{r comparisons-13-solution, exercise.reveal_solution = FALSE}
print(x, digits = 16)
```

```{r comparisons-13-check}
grade_this_code()
```
...
Type `near()` and within `near()`, add `x, c(1,2)`. This should come out as `TRUE` for both.
...
```{r comparisons-14-solution, exercise.reveal_solution = FALSE}
near(x, c(1, 2))
```

```{r comparisons-14-check}
grade_this_code()
```
...
```{r comparisons-15-solution, exercise.reveal_solution = FALSE}
age_mary <- NA
age_john <- NA
age_mary == age_john
```

```{r comparisons-15-check}
grade_this_code()
```
...
```{r comparisons-16-solution, exercise.reveal_solution = FALSE}
flights |>
  filter(dep_time == NA)
```

```{r comparisons-16-check}
grade_this_code()
```

### 

The following code doesn’t work correctly because `dep_time == NA` will yield `NA` for every single row, and `filter()` automatically drops missing values so the tibble has 0 observations. Instead we'll need a new tool: `is.na()`.

### Exercise 17

Type `is.na()` into the code chunk. Within `is.na()`, add `c(TRUE, NA, "a", 1)`.
...
```{r comparisons-17-solution, exercise.reveal_solution = FALSE}
is.na(c(TRUE, NA, "a", 1))
```

```{r comparisons-17-check}
grade_this_code()
```

### 

You will see the output `FALSE` for `TRUE`, `"a"`, `1` , but `TRUE` for `NA`. This is because `is.na()` can work with any type of vector and returns `TRUE` for missing values and `FALSE` for everything else.
...
```{r comparisons-18-solution, exercise.reveal_solution = FALSE}
flights |>
  filter(is.na(dep_time))
```

```{r comparisons-18-check}
grade_this_code()
```
...
```{r comparisons-19-solution, exercise.reveal_solution = FALSE}
flights |>
  filter(month == 1, day == 1)
```

```{r comparisons-19-check}
grade_this_code()
```
...
```{r comparisons-20-solution, exercise.reveal_solution = FALSE}
flights |>
  filter(month == 1, day == 1) |>
  arrange(dep_time)
```

```{r comparisons-20-check}
grade_this_code()
```
...
```{r comparisons-21-solution, exercise.reveal_solution = FALSE}
flights |>
  filter(month == 1, day == 1) |>
  arrange(desc(is.na(dep_time)), dep_time)
```

```{r comparisons-21-check}
grade_this_code()
```
...
The rules for missing values in Boolean algebra are a little tricky to explain because they seem inconsistent at first glance. Click "Submit Answer".
...
```{r boolean-algebra-1-solution, exercise.reveal_solution = FALSE}
df <- tibble(x = c(TRUE, FALSE, NA))
...
```{r boolean-algebra-2-solution, exercise.reveal_solution = FALSE}
flights |>
  filter(month == 11 | month == 12)
```

```{r boolean-algebra-2-check}
grade_this_code()
```
...
```{r boolean-algebra-3-solution, exercise.reveal_solution = FALSE}
flights |>
  filter(month == 11 | 12)
```

```{r boolean-algebra-3-check}
grade_this_code()
```
...
```{r boolean-algebra-4-solution, exercise.reveal_solution = FALSE}
flights |>
  mutate(nov = month == 11)
```

```{r boolean-algebra-4-check}
grade_this_code()
```
...
```{r boolean-algebra-5-solution, exercise.reveal_solution = FALSE}
flights |>
  mutate(
    nov = month == 11,
    final = nov | 12,
...
```{r boolean-algebra-5-check}
grade_this_code()
```

### Exercise 6

An easy way to avoid the problem of getting your `==`s and `|`s in the right order is to use `%in%`. `x %in% y` returns a logical vector the same length as `x` that is `TRUE` whenever a value in `x` is anywhere in `y`. `x %in% y` returns `TRUE` if anything in `x` is in `y`. For example, type `1:12 %in% c(1,5,11)` and click "Submit Answer". 
...
```{r boolean-algebra-6-solution, exercise.reveal_solution = FALSE}
1:12 %in% c(1, 5, 11)
```

```{r boolean-algebra-6-check}
grade_this_code()
```
...
```{r boolean-algebra-7-solution, exercise.reveal_solution = FALSE}
flights |>
  filter(month %in% c(11, 12))
```

```{r boolean-algebra-7-check}
grade_this_code()
```
...
Type `c(1, 2, NA) == NA` and click "Submit Answer".
...
```{r boolean-algebra-8-solution, exercise.reveal_solution = FALSE}
c(1, 2, NA) == NA
```

```{r boolean-algebra-8-check}
grade_this_code()
```
...
```{r boolean-algebra-9-solution, exercise.reveal_solution = FALSE}
c(1, 2, NA) %in% NA
```

```{r boolean-algebra-9-check}
grade_this_code()
```
...
```{r boolean-algebra-10-solution, exercise.reveal_solution = FALSE}
flights |>
  filter(dep_time %in% c(NA, 0800))
```

```{r boolean-algebra-10-check}
grade_this_code()
```
...
Start a pipe with `flights`. Pipe it into `summarize()`. Within `summarize()`, add `all_delayed = all(dep_delay <= 60)`. This will make a new column named `all_delayed` that is `TRUE` when `dep_delay` is less than or equal to 60 minutes for all flights.
...
```{r summaries-1-solution, exercise.reveal_solution = FALSE}
flights |>
...
```{r summaries-2-solution, exercise.reveal_solution = FALSE}
flights |>
...
Next, we will use `all()` and `any()` to find out if every flight was delayed on departure by at most an hour or if any flights were delayed on arrival by five hours or more. We can use the `.by` argument to do the calculations by day.

### Exercise 3

Copy the previous code. Within `summarize()`, add `any_long_delay = any(arr_delay >= 300, na.rm = TRUE)`. This will make a new column named `any_long_delay` that is `TRUE` when `arr_delay` is greater than or equal to `300` minutes.
...
flights |>
  summarize(
    ...
    any_long_delay = any(arr_delay >= ..., na.rm = TRUE)
  )
```

```{r summaries-3-solution, exercise.reveal_solution = FALSE}
flights |>
  summarize(
    all_delayed = all(dep_delay <= 60, na.rm = TRUE),
...
Like all summary functions, `any()` and `all()` will return `NA` if there are any missing values present. We added the argument `na.rm = TRUE` to make the missing values go away.

###

`>=` is used when you want to find a variable that is greater than or equal to a number. Conversely, you could use `<=` to find a variable that that is less than or equal to a number. Finally, `==` can be used to find a variable that is exactly equal to a number.

### Exercise 4

Copy the previous code. Add `.by = c(year, month, day)` to the `summarize()` function. Make sure to separate the arguments with commas. The added code will make the tibble have one row for each variation of `month`, `day` and `year`.
...
```{r summaries-4-solution, exercise.reveal_solution = FALSE}
flights |>
  summarize(
    all_delayed = all(dep_delay <= 60, na.rm = TRUE),
    any_long_delay = any(arr_delay >= 300, na.rm = TRUE),
...
Start a new pipe with `flights`. Pipe it into `summarize()`. Within `summarize()`, add `is_delayed = dep_delay <= 60`.
...
flights |>
  summarize(
    is_delayed = dep_delay <= ...
  )
```

```{r summaries-5-solution, exercise.reveal_solution = FALSE}
flights |>
  summarize(
    is_delayed = dep_delay <= 60
  )
```
...
This should create a column called `is_delayed` that is `TRUE` when `dep_delay` is less than or equal to `60` and `FALSE` when it is over `60`. There should also be a warning since there should only be 1 value returned, but we did something that returned an output per row so `mutate()` should have been used. The point here is to see we have a column of `TRUE`s and `FALSE`s (a logical vector which we will apply some function to).

### Exercise 6

Copy the previous code. Change `is_delayed = dep_delay <= 60` to be `prop_delayed = mean(dep_delay <= 60)`.
...
flights |>
  summarize(
    prop_delayed = ...(dep_delay <= 60)
  )
```

```{r summaries-6-solution, exercise.reveal_solution = FALSE}
flights |>
  summarize(
    prop_delayed = mean(dep_delay <= 60)
  )
```
...
This created a new column called `prop_delayed`. We need to set `na.rm` equal to `TRUE` to remove all `NA` values.
...
flights |>
  summarize(prop_delayed = mean(dep_delay <= 60, ...))
```

```{r summaries-7-solution, exercise.reveal_solution = FALSE}
flights |>
  summarize(
...
In computations, `TRUE` is equal to `1` and `FALSE` is equal to `0`. Therefore the `mean` is equal to the proportion of flights delayed less than `60` minutes.

### Exercise 8

Copy the previous code and add `count_long_delay = sum(arr_delay >= 300, na.rm = TRUE)` as a new argument in `summarize()`. Again, make sure to separate the arguments with commas.
...
```{r summaries-8-solution, exercise.reveal_solution = FALSE}
flights |>
  summarize(
    prop_delayed = mean(dep_delay <= 60, na.rm = TRUE),
...
Copy the previous code and add `.by = c(year, month, day)` as a new argument within `summarize()` to get rows for every single day of the year.
...
```{r summaries-9-solution, exercise.reveal_solution = FALSE}
flights |>
  summarize(
    prop_delayed = mean(dep_delay <= 60, na.rm = TRUE),
    count_long_delay = sum(arr_delay >= 300, na.rm = TRUE),
...
This allows for us to see the proportion of flights that were delayed on departure by at most 1 hour (`60` minutes) and the number of flights that were delayed on arrival by 5 hours (`300` minutes) or more for each day in the dataset.
...
```{r summaries-10-solution, exercise.reveal_solution = FALSE}
flights |>
  filter(arr_delay > 0)
```

```{r summaries-10-check}
grade_this_code()
```
...
```{r summaries-11-solution, exercise.reveal_solution = FALSE}
flights |>
  filter(arr_delay > 0) |>
  summarize(behind = mean(arr_delay))
```

```{r summaries-11-check}
grade_this_code()
```
...
```{r summaries-12-solution, exercise.reveal_solution = FALSE}
flights |>
  filter(arr_delay > 0) |>
  summarize(
    behind = mean(arr_delay),
...
```{r summaries-13-solution, exercise.reveal_solution = FALSE}
flights |>
  filter(arr_delay > 0) |>
  summarize(
    behind = mean(arr_delay),
...
```{r summaries-14-solution, exercise.reveal_solution = FALSE}
flights |>
  summarize(
    behind = mean(arr_delay[arr_delay > 0], na.rm = TRUE),
    ahead = mean(arr_delay[arr_delay < 0], na.rm = TRUE),
...
Let’s begin with a simple example of labeling a numeric vector as either `“+ve”` (positive) or `“-ve”` (negative). Assign `x` to the vector `c(-3:3, NA)` and press "Submit Answer".
...
```{r conditional-transfor-1-solution, exercise.reveal_solution = FALSE}
x <- c(-3:3, NA)
```

```{r conditional-transfor-1-check}
grade_this_code()
```
...
```{r conditional-transfor-2-solution, exercise.reveal_solution = FALSE}
x <- c(-3:3, NA)
if_else(x > 0, "+ve", "-ve")
```

```{r conditional-transfor-2-check}
grade_this_code()
```
...
```{r conditional-transfor-3-solution, exercise.reveal_solution = FALSE}
x <- c(-3:3, NA)
if_else(x > 0, "+ve", "-ve", "???")
```

```{r conditional-transfor-3-check}
grade_this_code()
```
...
You can also use vectors for the the `TRUE` and `FALSE` arguments. Let us attempt to create a minimal implementation of `abs()`. Below the code assigning a vector to `x`, add `if_else(x < 0, -x, x)`. Click "Submit Answer". 
...
```{r conditional-transfor-4-solution, exercise.reveal_solution = FALSE}
x <- c(-3:3, NA)
if_else(x < 0, -x, x)
```

```{r conditional-transfor-4-check}
grade_this_code()
```
...
For example, you could implement a simple version of `coalesce()`. Below the code assigning vectors to `x1` and `y1`, type `if_else(is.na(x1), y1, x1)` and click "Submit Answer".
...
```{r conditional-transfor-5-solution, exercise.reveal_solution = FALSE}
x1 <- c(NA, 1, 2, NA)
y1 <- c(3, NA, 4, 6)
if_else(is.na(x1), y1, x1)
```

```{r conditional-transfor-5-check}
grade_this_code()
```
...
Below the provided code, type `if_else(x == 0, "0", if_else(x < 0, "-ve", "+ve"), "???")` and click "Submit Answer".
...
```{r conditional-transfor-6-solution, exercise.reveal_solution = FALSE}
x <- c(-3:3, NA)
if_else(x == 0, "0", if_else(x < 0, "-ve", "+ve"), "???")
```

```{r conditional-transfor-6-check}
grade_this_code()
```
###
...
The code below should have the same output as above, but instead of using `if_else`, we are using `case_when()`. Click "Submit Answer".
...
```{r conditional-transfor-7-solution, exercise.reveal_solution = FALSE}
x <- c(-3:3, NA)
case_when(
  x == 0 ~ "0",
  x < 0 ~ "-ve",
...
```{r conditional-transfor-7-check}
grade_this_code()
```
...
To explain how `case_when()` works, let’s explore some simpler cases. If none of the cases match, the output gets an `NA`. Click "Submit Answer".
...
```{r conditional-transfor-8-solution, exercise.reveal_solution = FALSE}
x <- c(-3:3, NA)
case_when(
  x < 0 ~ "-ve",
  x > 0 ~ "+ve"
...
```{r conditional-transfor-8-check}
grade_this_code()
```
...
If you want to create a “default”/catch all value, use TRUE on the left hand side. Copy the previous code and add `TRUE ~ "???"` as a new condition. Make sure to separate the arguments with a comma.
...
```{r conditional-transfor-9-solution, exercise.reveal_solution = FALSE}
x <- c(-3:3, NA)
case_when(
  x < 0 ~ "-ve",
  x > 0 ~ "+ve",
...
```{r conditional-transfor-9-check}
grade_this_code()
```
...
Also, note that if multiple conditions match, only the first will be used. Click "Submit Answer".
...
```{r conditional-transfor-10-solution, exercise.reveal_solution = FALSE}
x <- c(-3:3, NA)
case_when(
  x > 0 ~ "+ve",
  x > 2 ~ "big"
...
```{r conditional-transfor-10-check}
grade_this_code()
```
...
For example, we could use `case_when()` to provide some human readable labels for the arrival delay. Your gratitude for having the code written for you is much appreciated. Click `Submit code`.
...
```{r conditional-transfor-11-solution, exercise.reveal_solution = FALSE}
flights |>
  mutate(
    status = case_when(
      is.na(arr_delay) ~ "cancelled",
...
```{r conditional-transfor-11-check}
grade_this_code()
```

<!-- could not figure out how to do grading with an error as the intended result -->
...
<!-- Note that both `if_else()` and `case_when()` require compatible types in the output. If they’re not compatible, you’ll see errors. To demonstrate, click "Submit Answer". -->

<!-- ```{r conditional-transfor-12, exercise = TRUE} -->
<!-- if_else(TRUE, "a", 1) -->
<!-- ``` -->

<!-- ```{r conditional-transfor-12-error-solution} -->

<!-- ``` -->

<!-- ```{r conditional-transfor-12-error-check} -->
<!-- grade_this_code() -->
<!-- ``` -->
