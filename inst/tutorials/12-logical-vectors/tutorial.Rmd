...
The rules for missing values in Boolean algebra are a little tricky to explain because they seem inconsistent at first glance. Click "Submit Code".
...
This should create a column called `is_delayed` that is `TRUE` when `dep_delay` is less than or equal to `60` and `FALSE` when it is over `60`.
...
flights|>
  summarize(prop_delayed = mean(dep_delay <= 60, na.rm = TRUE),
            count_long_delay = ...)
```
...
The code below should have the same output as above, but instead of using `if_else`, we are using `case_when()`. Click "Submit Code".
...
To explain how `case_when()` works, let’s explore some simpler cases. If none of the cases match, the output gets an `NA`. Click "Submit Code".
...
Also, note that if multiple conditions match, only the first will be used. Click "Submit Code".
...
Note that both `if_else()` and `case_when()` require compatible types in the output. If they’re not compatible, you’ll see errors. To demonstrate, click "Submit Code".
...
```{r conditional-transfor-12-solution, exercise.reveal_solution = FALSE}
if_else(TRUE, "a", 1)
```

```{r conditional-transfor-12-check}
grade_this_code()
```
