...
The easiest place to start when turning an exploratory graphic into an expository graphic is with good labels. You add labels with the `labs()` function. We have used this in previous tutorials to create a `title`, `subtitle`, `x` axis label, and `y` axis label.
...
- the **tidyverse** package using the `library()` function. 
- the [**scales**](https://scales.r-lib.org/) package using the `library()` function. 
- the [**ggrepel**](https://ggrepel.slowkow.com) package using `library()`. 
- the [**patchwork**](https://patchwork.data-imaginist.com) package using `library()`. 
...
```{r labels-1-solution, exercise.reveal_solution = FALSE}
library(tidyverse)
library(scales)
library(ggrepel)
library(patchwork)
...
Let's start with an example to learn how to add more advanced labels with the `quote` function. 

Type in `labels_tib` and hit "Submit Answer". 
...
```{r labels-2-solution, exercise.reveal_solution = FALSE}
labels_tib
```
...
Within `ggplot()`, using `aes()`, set the `x` argument to the `start` variable and set the `y` argument to the `end` variable.
...
```{r labels-3-solution, exercise.reveal_solution = FALSE}
labels_tib |> 
...
```{r labels-4-solution, exercise.reveal_solution = FALSE}
labels_tib |> 
  ggplot(aes(x = start, y = end)) + 
  geom_point()
```
...
Using `labs()`, set the `title` to `"Exponential relationship between 'x' and the cumulative sum of 'x' squared"`.
...
```{r labels-5-solution, exercise.reveal_solution = FALSE}
labels_tib |> 
  ggplot(aes(x = start, y = end)) + 
  geom_point() +
  labs(
    title = "Exponential relationship between 'x' and the cumulative sum of 'x' squared"
  )
```
...
The purpose of a plot title is to summarize the main finding. Avoid titles that just describe what the plot is, e.g., “A scatterplot of x vs. cumulative sum of x squared”.

###

A `subtitle` adds additional detail in a smaller font beneath the title. If there is one key conclusion which readers should come away with, spell it out in the subtitle.


### Exercise 6

Within `labs()`, set the `caption` to `"Synthetic calculated dataset"`.
...
```{r labels-6-solution, exercise.reveal_solution = FALSE}
labels_tib |> 
  ggplot(aes(x = start, y = end)) + 
  geom_point() +
  labs(
    title = "Exponential relationship between 'x' and the cumulative sum of 'x' squared",
    caption = "Synthetic calculated dataset"
  )
```
...
Within `labs()`, set the `x` axis title to `quote(x[i])`.
...
```{r labels-7-solution, exercise.reveal_solution = FALSE}
labels_tib |> 
  ggplot(aes(x = start, y = end)) + 
  geom_point() +
  labs(
...
```{r labels-8-solution, exercise.reveal_solution = FALSE}
labels_tib |> 
  ggplot(aes(x = start, y = end)) + 
  geom_point() +
  labs(
...
```{r labels-9}
question_text(NULL,answer_fn(function(value){
                              if(length(value) >= 1 ) {
                                return(mark_as(TRUE))
                                }
                              return(mark_as(FALSE) )
...
There are two possible sources of labels. First, you might have a tibble that provides labels. Type `label_info` and hit "Submit Answer".
...
```{r annotations-1-solution, exercise.reveal_solution = FALSE}
label_info
```

```{r annotations-1-check}
grade_this_code()
```
...
Create a new pipeline. Pipe `mpg` to `ggplot()`. Using `aes()`, map `x` to `displ` and `y` to `hwy`. Add `geom_point()`.
...
```{r annotations-2-solution, exercise.reveal_solution = FALSE}
mpg |>   
  ggplot(aes(x = displ, y = hwy)) +
    geom_point()
```

```{r annotations-2-check}
grade_this_code()
```
...
```{r annotations-3-solution, exercise.reveal_solution = FALSE}
mpg |>   
  ggplot(aes(x = displ, y = hwy, color = drv)) +
  geom_point()
```

```{r annotations-3-check}
grade_this_code()
```
...
```{r annotations-4-solution, exercise.reveal_solution = FALSE}
mpg |>   
  ggplot(aes(x = displ, y = hwy, color = drv)) +
    geom_point(alpha = 0.3)
```

```{r annotations-4-check}
grade_this_code()
```
...
```{r annotations-5-solution, exercise.reveal_solution = FALSE}
mpg |>   
  ggplot(aes(x = displ, y = hwy, color = drv)) +
    geom_point(alpha = 0.3) +
    geom_smooth(method = "loess", formula = y ~ x, se = FALSE)
...
```{r annotations-6-solution, exercise.reveal_solution = FALSE}
mpg |>   
  ggplot(aes(x = displ, y = hwy, color = drv)) +
    geom_point(alpha = 0.3) +
    geom_smooth(method = "loess", formula = y ~ x, se = FALSE) +
...
```{r annotations-7-solution, exercise.reveal_solution = FALSE}
mpg |>   
  ggplot(aes(x = displ, y = hwy, color = drv)) +
    geom_point(alpha = 0.3) +
    geom_smooth(method = "loess", formula = y ~ x, se = FALSE) +
...
```{r annotations-8-solution, exercise.reveal_solution = FALSE}
mpg |>   
  ggplot(aes(x = displ, y = hwy, color = drv)) +
  geom_point(alpha = 0.3) +
  geom_smooth(method = "loess", formula = y ~ x, se = FALSE) +
...
```{r annotations-9-solution, exercise.reveal_solution = FALSE}
mpg |>   
  ggplot(aes(x = displ, y = hwy, color = drv)) +
  geom_point(alpha = 0.3) +
  geom_smooth(method = "loess", formula = y ~ x, se = FALSE) +
...
```{r annotations-10-solution, exercise.reveal_solution = FALSE}
mpg |>   
  ggplot(aes(x = displ, y = hwy, color = drv)) +
  geom_point(alpha = 0.3) +
  geom_smooth(method = "loess", formula = y ~ x, se = FALSE) +
...
Let's create a scatterplot of the relationship between highway miles per gallon and displacement again, only this time let's emphasize the outliers.


Type `potential_outliers` and hit "Submit Answer". 
...
```{r annotations-11-solution, exercise.reveal_solution = FALSE}
potential_outliers
```

```{r annotations-11-check}
grade_this_code()
```
...
Before identifying these outliers in the graphic, we need to set up our initial plot. 

With a new pipeline, pipe `mpg` to `ggplot()`. Within this `ggplot()`, using `aes()`, map `x` to `displ` and `y` to `hwy`. Add `geom_point()` so that the data is displayed. 
...
```{r annotations-12-solution, exercise.reveal_solution = FALSE}
mpg |> 
  ggplot(aes(x = displ, y = hwy)) +
  geom_point()
```

```{r annotations-12-check}
grade_this_code()
```
...
```{r annotations-13-solution, exercise.reveal_solution = FALSE}
mpg |> 
  ggplot(aes(x = displ, y = hwy)) +
  geom_point() +
  geom_text_repel(
...
```{r annotations-14-solution, exercise.reveal_solution = FALSE}
mpg |> 
  ggplot(aes(x = displ, y = hwy)) +
  geom_point() +
  geom_text_repel(
...
```{r annotations-15-solution, exercise.reveal_solution = FALSE}
mpg |>
  ggplot(aes(x = displ, y = hwy)) +
  geom_point() +
  geom_text_repel(
...
```{r annotations-16-solution, exercise.reveal_solution = FALSE}
trend_text <- "Larger engine sizes tend to have lower fuel economy."
```

```{r annotations-16-check}
grade_this_code()
```
...
```{r annotations-17-solution, exercise.reveal_solution = FALSE}
trend_text <- "Larger engine sizes tend to have lower fuel economy." |> 
  str_wrap(width = 30)
```

```{r annotations-17-check}
grade_this_code()
```

### Exercise 18

Now let's make use of this annotation within a plot. 

Create a new pipeline and pipe `mpg` to the `ggplot()` function. Within this function map `x` to `displ` and `y` to `hwy` using `aes()`. Add the `geom_point()` function to the pipeline.
...
```{r annotations-18-solution, exercise.reveal_solution = FALSE}
mpg |> 
  ggplot(aes(x = displ, y = hwy)) +
  geom_point()
```

```{r annotations-18-check}
grade_this_code()
```
...
```{r annotations-19-solution, exercise.reveal_solution = FALSE}
mpg |>
  ggplot(aes(x = displ, y = hwy)) +
  geom_point() +
  annotate(
...
```{r annotations-20-solution, exercise.reveal_solution = FALSE}
mpg |>
  ggplot(aes(x = displ, y = hwy)) +
  geom_point() +
  annotate(
...
```{r scales-1-solution, exercise.reveal_solution = FALSE}
mpg |> 
  ggplot(aes(x = displ, y = hwy)) +
    geom_point(aes(color = class))
```
...
```{r scales-2-solution, exercise.reveal_solution = FALSE}
mpg |> 
  ggplot(aes(x = displ, y = hwy)) +
    geom_point(aes(color = class)) +
    scale_x_continuous() +
...
```{r scales-3-solution, exercise.reveal_solution = FALSE}
mpg |> 
  ggplot(aes(x = displ, y = hwy, color = drv)) +
  geom_point()
```
...
```{r scales-4-solution, exercise.reveal_solution = FALSE}
mpg |> 
  ggplot(aes(x = displ, y = hwy, color = drv)) +
    geom_point() +
...
```{r scales-5-solution, exercise.reveal_solution = FALSE}
mpg |> 
  ggplot(aes(x = displ, y = hwy, color = drv)) +
    geom_point() +
    scale_y_continuous(breaks = seq(15, 40, by = 5)) +
...
```{r scales-6-solution, exercise.reveal_solution = FALSE}
mpg |> 
  ggplot(aes(x = displ, y = hwy, color = drv)) +
    geom_point() +
    scale_y_continuous(breaks = seq(15, 40, by = 5)) +
...
This time we will be using the **scales** package to create more efficient axis labels. 

Create a new pipeline and pipe `diamonds` to the `ggplot()` function. Within this function, pipe map `x` to `price` and `y` to `cut`. Add the `geom_boxplot()` function to the pipeline. Within this function add the `alpha` argument and set it to `0.05`. 
...
```{r scales-7-solution, exercise.reveal_solution = FALSE}
diamonds |> 
...
```{r scales-8-solution, exercise.reveal_solution = FALSE}
diamonds |> 
  ggplot(aes(x = price, y = cut)) +
  geom_boxplot(alpha = 0.05) +
...
Create a new pipeline and pipe `diamonds` to the `ggplot()` function. Within this function, map `x` to `cut` and `fill` to `clarity`. Add the `geom_bar()` function. Within this function add the `position` argument and set it equal to `"fill"`.
...
```{r scales-9-solution, exercise.reveal_solution = FALSE}
diamonds |>
...
```{r scales-10-solution, exercise.reveal_solution = FALSE}
diamonds |>
  ggplot(aes(x = cut, fill = clarity)) +
    geom_bar(position = "fill") +
...
Type in `presidential` and hit "Submit Answer".
...
```{r scales-11-solution, exercise.reveal_solution = FALSE}
presidential
```
...
```{r scales-12-solution, exercise.reveal_solution = FALSE}
presidential |> 
...
`id` is simply the number of the presidency is that row. Eisenhower was the 34th President (1st row in data), Kennedy was the 35th (2nd row in data), and so on.
...
```{r scales-13-solution, exercise.reveal_solution = FALSE}
presidential |> 
  mutate(id = 33 + row_number()) |> 
  ggplot(aes(x = start, y = id)) +
  geom_point() +
...
```{r scales-14-solution, exercise.reveal_solution = FALSE}
presidential |> 
  mutate(id = 33 + row_number()) |> 
  ggplot(aes(x = start, y = id)) +
  geom_point() +
...
```{r scales-15-solution, exercise.reveal_solution = FALSE}
presidential |> 
  mutate(id = 33 + row_number()) |> 
  ggplot(aes(x = start, y = id)) +
  geom_point() +
...
```{r scales-16-solution, exercise.reveal_solution = FALSE}
presidential |>
  mutate(id = 33 + row_number()) |>
  ggplot(aes(x = start, y = id)) +
  geom_point() +
...
First create a new ggplot object called `base`. 

Assign `base` as the dataset `mpg` piped to the `ggplot()` function. Within the `aes()` for this function map `x` to `displ` and `y` to `hwy`.  Add `geom_point()` with `aes(color = class)`
...
```{r legend-layout-1-solution, exercise.reveal_solution = FALSE}
base <- mpg |> 
  ggplot(aes(x = displ, y = hwy)) +
  geom_point(aes(color = class))
```

```{r legend-layout-1-check}
grade_this_code()
```
...
```{r legend-layout-2-solution, exercise.reveal_solution = FALSE}
base +
  theme(legend.position = "left")
```

```{r legend-layout-2-check}
grade_this_code()
```
...
```{r legend-layout-3-solution, exercise.reveal_solution = FALSE}
base +
  theme(legend.position = "top")
```

```{r legend-layout-3-check}
grade_this_code()
```
...
```{r legend-layout-4-solution, exercise.reveal_solution = FALSE}
base +
  theme(legend.position = "top") +
  guides(color = guide_legend(nrow = 3))
```

```{r legend-layout-4-check}
grade_this_code()
```
...
We will be working with the familiar `diamonds` dataset. As a reminder of whats in the dataset, type `diamonds` and hit "Submit Answer".
...
```{r replacing-a-scale-1-solution, exercise.reveal_solution = FALSE}
diamonds
```

```{r replacing-a-scale-1-check}
grade_this_code()
```
...
```{r replacing-a-scale-2-solution, exercise.reveal_solution = FALSE}
diamonds |> 
  ggplot(aes(x = carat, y = price))
```

```{r replacing-a-scale-2-check}
grade_this_code()
```

### Exercise 3

Using your previous code, add the `geom_bin_2d()` function to the pipeline.
...
```{r replacing-a-scale-3-solution, exercise.reveal_solution = FALSE}
diamonds |> 
  ggplot(aes(x = carat, y = price)) +
    geom_bin_2d()
```

```{r replacing-a-scale-3-check}
grade_this_code()
```
...
```{r replacing-a-scale-4-solution, exercise.reveal_solution = FALSE}
diamonds |> 
  ggplot(aes(x = log10(carat), y = log10(price))) +
  geom_bin_2d()
```

```{r replacing-a-scale-4-check}
grade_this_code()
```
...
```{r replacing-a-scale-5-solution, exercise.reveal_solution = FALSE}
diamonds |> 
  ggplot(aes(x = carat, y = price)) +
  geom_bin_2d() +
  scale_x_log10()
...
```{r replacing-a-scale-6-solution, exercise.reveal_solution = FALSE}
diamonds |> 
  ggplot(aes(x = carat, y = price)) +
    geom_bin_2d() +
    scale_x_log10() +
...
Start a new pipeline with `mpg` piped into the `ggplot()` function. Within this function map `x` to `displ` and `y` to `hwy`. Add `geom_point()` function to the pipeline with `aes()` and `color` equal to `drv`.
...
```{r replacing-a-scale-7-solution, exercise.reveal_solution = FALSE}
mpg |> 
  ggplot(aes(x = displ, y = hwy)) +
  geom_point(aes(color = drv))
```

```{r replacing-a-scale-7-check}
grade_this_code()
```
...
```{r replacing-a-scale-8-solution, exercise.reveal_solution = FALSE}
mpg |> 
  ggplot((aes(x = displ, y = hwy))) +
    geom_point(aes(color = drv)) +
...
```{r replacing-a-scale-9-solution, exercise.reveal_solution = FALSE}
mpg |> 
  ggplot((aes(x = displ, y = hwy))) +
...
- Start a new pipeline with the dataset `presidential`piped into the `mutate()` function. 
- Within the `mutate()` function, create a new variable called `id` and set it equal to `33` plus `row_number()`. 
- Add the `ggplot()` function to the pipeline and map `x` to `start`, `y` to `id`, and `color` to `party`. 
- Add `geom_point()`.
...
```{r replacing-a-scale-10-solution, exercise.reveal_solution = FALSE}
presidential |> 
  mutate(id = 33 + row_number()) |> 
...
```{r replacing-a-scale-11-solution, exercise.reveal_solution = FALSE}
presidential |> 
  mutate(id = 33 + row_number()) |> 
  ggplot(aes(x = start, y = id, color = party)) +
  geom_point() +
...
```{r replacing-a-scale-12-solution, exercise.reveal_solution = FALSE}
presidential |> 
  mutate(id = 33 + row_number()) |> 
  ggplot(aes(x = start, y = id, color = party)) +
  geom_point() +
...
For our next plot, we will create a plot with the dataset `random_vals`. Type in `random_vals` and hit "Submit Answer".
...
```{r replacing-a-scale-13-solution, exercise.reveal_solution = FALSE}
random_vals
```

```{r replacing-a-scale-13-check}
grade_this_code()
```
...
```{r replacing-a-scale-14-solution, exercise.reveal_solution = FALSE}
random_vals |> 
...
```{r replacing-a-scale-15-solution, exercise.reveal_solution = FALSE}
random_vals |> 
  ggplot(aes(x = random_x, y = random_y)) +
  geom_hex() +
  coord_fixed() +
...
```{r replacing-a-scale-16-solution, exercise.reveal_solution = FALSE}
random_vals |> 
  ggplot(aes(x = random_x, y = random_y)) +
  geom_hex() +
  coord_fixed() +
...
- Pipe `mpg` to `ggplot()`, using `aes()`, map `x` to `displ` and `y` to `hwy`. 
...
```{r zooming-1-solution, exercise.reveal_solution = FALSE}
mpg |> 
  ggplot(aes(x = displ, y = hwy)) +
  geom_point(aes(color = drv)) +
  geom_smooth()
...
```{r zooming-2-solution, exercise.reveal_solution = FALSE}
## option 1 ----
mpg |> 
...
```{r zooming-3-solution, exercise.reveal_solution = FALSE}
mpg |> 
  ggplot(aes(x = displ, y = hwy)) +
  geom_point(aes(color = drv)) +
  geom_smooth() +
...
```{r zooming-4-solution, exercise.reveal_solution = FALSE}
mpg |>
  ggplot(aes(x = displ, y = hwy)) +
  geom_point(aes(color = drv)) +
  geom_smooth() +
...
- Pipe `mpg` to `ggplot()`. 
- Within `aes()`, set `x = displ` and `y = hwy`. 
...
```{r themes-1-solution, exercise.reveal_solution = FALSE}
mpg |> 
  ggplot(aes(x = displ, y = hwy)) +
  geom_point(aes(color = class)) +
  geom_smooth(se = FALSE)
...
This plot uses the `theme_gray()` default. 
...
```{r themes-2-solution, exercise.reveal_solution = FALSE}
mpg |> 
  ggplot(aes(x = displ, y = hwy)) +
  geom_point(aes(color = class)) +
  geom_smooth(se = FALSE) +
...
```{r themes-3-solution, exercise.reveal_solution = FALSE}
mpg |> 
  ggplot(aes(x = displ, y = hwy)) +
  geom_point(aes(color = class)) +
  geom_smooth(se = FALSE) +
...
```{r themes-4-solution, exercise.reveal_solution = FALSE}
mpg |> 
  ggplot(aes(x = displ, y = hwy)) +
  geom_point(aes(color = class)) +
  geom_smooth(se = FALSE) +
...
```{r themes-5-solution, exercise.reveal_solution = FALSE}
ggplot(mpg, aes(x = displ, y = hwy, color = drv)) +
  geom_point()
```
...
```{r themes-6-solution, exercise.reveal_solution = FALSE}
ggplot(mpg, aes(x = displ, y = hwy, color = drv)) +
  geom_point() +
  labs(
    title = "Larger engine sizes tend to have lower fuel economy",
...
Add `theme()` to the pipeline. Within `theme()`, add `legend.position` set to `"inside"` and `legend.position.inside` set `c(0.6, 0.7)`.
...
```{r themes-7-solution, exercise.reveal_solution = FALSE}
ggplot(mpg, aes(x = displ, y = hwy, color = drv)) +
  geom_point() +
  labs(
    title = "Larger engine sizes tend to have lower fuel economy",
...
The `legend.position` argument sets the position of legends ("none", "left", "right", "bottom", "top", or "inside").
...
```{r themes-8-solution, exercise.reveal_solution = FALSE}
ggplot(mpg, aes(x = displ, y = hwy, color = drv)) +
  geom_point() +
  labs(
    title = "Larger engine sizes tend to have lower fuel economy",
...
```{r themes-9-solution, exercise.reveal_solution = FALSE}
ggplot(mpg, aes(x = displ, y = hwy, color = drv)) +
  geom_point() +
  labs(
    title = "Larger engine sizes tend to have lower fuel economy",
...
```{r themes-10-solution, exercise.reveal_solution = FALSE}
ggplot(mpg, aes(x = displ, y = hwy, color = drv)) +
  geom_point() +
  labs(
    title = "Larger engine sizes tend to have lower fuel economy",
...
```{r themes-11-solution, exercise.reveal_solution = FALSE}
ggplot(mpg, aes(x = displ, y = hwy, color = drv)) +
  geom_point() +
  labs(
    title = "Larger engine sizes tend to have lower fuel economy",
...
We have created several plots (`p1` through `p8`) for you to use in these exercises. Type `p1` and hit "Submit Answer".
...
```{r layout-1-solution, exercise.reveal_solution = FALSE}
p1
```
...
Enter `p1` on the first line and `p2` on the second line. Hit "Submit Answer".
...
```{r layout-2-solution, exercise.reveal_solution = FALSE}
p1
p2
```
...
Type `p1 + p2` and hit "Submit Answer".
...
```{r layout-3-solution, exercise.reveal_solution = FALSE}
p1 + p2
```
...
You can also create complex plot layouts with **patchwork**. Type `(p1 | p3) / p2` and hit "Submit Answer".
...
```{r layout-4-solution, exercise.reveal_solution = FALSE}
(p1 | p3) / p2
```
...
**Note:** For the remaining exercises/plots we have increased the size of the plotting window to have a height of 8 inches. We have done this since we are building a graphic with many plots in it so we need more area to work with.
...
Start with `(p1 + p2) / (p3 + p4) / p5`.  

```{r layout-5, exercise = TRUE, fig.height=8}
...
```{r layout-5-solution, exercise.reveal_solution = FALSE}
(p1 + p2) / (p3 + p4) / p5
```
...
```{r layout-6, exercise = TRUE, fig.height=8}
...
```{r layout-6-solution, exercise.reveal_solution = FALSE}
...
None of our plots, by design, include a legend because we want a single legend for the entire diplay. We also want a single title. 

To add this, we need to use the **patchwork** function `plot_annotation()`. Add it to our plot, setting the `title` argument to `"City and highway mileage for cars with different drive trains"` and the `caption` argument to `"Source: https://fueleconomy.gov"`. 

```{r layout-7, exercise = TRUE, fig.height=8}
...
```{r layout-7-solution, exercise.reveal_solution = FALSE}
guide_area() / (p1 + p2) / (p3 + p4) / p5 +
  plot_annotation(
      title = "City and highway mileage for cars with different drive trains",
      caption = "Source: https://fueleconomy.gov"
...
`plot_layout()` provides fine-grained control over the details of how the plots are put together. 

Add `plot_layout()` with the `guides` argument set to `"collect"` and the `heights` argument set to `c(1, 3, 2, 4)`.

```{r layout-8, exercise = TRUE, fig.height=8}
...
```{r layout-8-solution, exercise.reveal_solution = FALSE}
guide_area() / (p1 + p2) / (p3 + p4) / p5 +
  plot_annotation(
      title = "City and highway mileage for cars with different drive trains",
      caption = "Source: https://fueleconomy.gov"
...
```{r layout-9, exercise = TRUE, fig.height=8}
...
```{r layout-9-solution, exercise.reveal_solution = FALSE}
guide_area() / (p1 + p2) / (p3 + p4) / p5 +
    plot_annotation(
        title = "City and highway mileage for cars with different drive trains",
        caption = "Source: https://fueleconomy.gov"
