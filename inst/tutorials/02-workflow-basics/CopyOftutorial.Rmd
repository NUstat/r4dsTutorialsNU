...
This should be the same as the version you saw at the top left of the Console pane.
...
If this returns `TRUE`, then you have a "typical" computer set up, with a Desktop in your home directory. (The tilde symbol, `~`, refers to a user's home directory.) We recommend that you store your RStudio projects there. However, more experienced computer users should feel free to ignore this advice. If the return value is `FALSE`, you will need to create a location by hand.
...
<!-- Would it make sense to learn how to do this from the Console? Looks like: -->

<!-- usethis::create_project("~/Desktop/projects/test") -->

<!-- does all the necessary magic, including creating the new directory would do the trick. But I dislike this option because it creates a useless R directory in the project, which can only confuse things. I am also suspicious of the `.Rproj.user` directory which is created.  -->
...
Yes, it is confusing that this is the second time you have selected `New Project`! Use `analysis-1` for the "Directory name". In the "Create project as subdirectory of" box, use your new `projects` folder.
...
Indeed, the *definition* of an R project --- from the point of view of RStudio --- is a directory with an associated `Rproj` file. (A "folder" and a "directory" are the same thing. Experienced users prefer "directory.")
...
<!-- DK: Anything more we might do here? Maybe restart R session, see x disappear and source whole script? We would then need to edit one comment/question in the next session. -->
...
Click `File -> New File -> R Script`. At the top of the Source pane, the file should be called `Untitled1`. 
...
You will see that `script-2.R`, the name of the file, is now red, with an asterisk. This indicates that there are unsaved changes in the file.
...
Run the entire file again. Note the sloppiness of the word "run" in this instruction. It is not always clear what "run" means. For example, it does *not* mean to click the "Run" button! In general, "run" means to "execute" all the code in the file, most commonly by either using `Command/Ctrl + Shift + Enter` or by pressing the "Source" button.
...
Run the file. Again, to "run" a script generally means to "execute" it, meaning sending it to the R process. We have learned two ways to (easily) execute an entire file: pressing the "Source" button or hitting the associated shortcut key: `Command/Ctrl + Shift + Enter`.
...
Do so. Run the file. Note that you don't need to save the file first. If you execute a file with either "Source" or `Command/Ctrl + Shift + Enter`, RStudio will save the file automatically.
