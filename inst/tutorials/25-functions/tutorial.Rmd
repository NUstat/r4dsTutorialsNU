...
Start a pipe with `df1` to `grouped_mean()` and pass in `group` and `x` and to clarify, this data set has a column named `group_var`
...
Start a pipe with `dimaonds` to `count_prop()` and pass in `clarity` as the argument.
