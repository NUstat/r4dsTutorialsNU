...
description: 'Tutorial for Chapter 25: Functions'
---

<!-- Modification of r4ds.tutorials author: Anish Bellamkonda-->
...
grade_server("grade",
  exclude = c(
    "simple-function-10",
    "simple-function-11",
    "data-frame-functions-3",
    "data-frame-functions-18"
  )
)
```
...
```{r simple-function-2-solution, exercise.reveal_solution = FALSE}
myfunc_1 <- function(){
  
}
```

```{r simple-function-2-check}
grade_this_code()
```
...
```{r simple-function-3-solution, exercise.reveal_solution = FALSE}
myfunc_1()
```

```{r simple-function-3-check}
grade_this_code()
```
...
```{r simple-function-4-solution, exercise.reveal_solution = FALSE}
myfunc_1
```

```{r simple-function-4-check}
grade_this_code()
```
...
```{r simple-function-5-solution, exercise.reveal_solution = FALSE}
myfunc_2 <- function(x){
  
}
```

```{r simple-function-5-check}
grade_this_code()
```

### 

This is the same as `myfunc_1` but it takes the argument `x`.

### Exercise 6

Run `myfunc_2()` and pass in the number `5`.
...
```{r simple-function-6-solution, exercise.reveal_solution = FALSE}
myfunc_2(5)
```

```{r simple-function-6-check}
grade_this_code()
```
...
```{r simple-function-7-solution, exercise.reveal_solution = FALSE}
myfunc_2()
```

```{r simple-function-7-check}
grade_this_code()
```
...
```{r simple-function-8-solution, exercise.reveal_solution = FALSE}
myfunc_3 <- function(x){
  x^2
}
```

```{r simple-function-8-check}
grade_this_code()
```
...
Let's now use the function, so run `myfunc_3()` and pass in the number `3`.
...
```{r simple-function-9-solution, exercise.reveal_solution = FALSE}
myfunc_3(3)
```

```{r simple-function-9-check}
grade_this_code()
```
...
<!-- ```{r simple-function-10-solution, exercise.reveal_solution = FALSE} -->
<!-- myfunc_3("abc") -->
<!-- ``` -->

<!-- ```{r simple-function-10-check} -->
<!-- grade_this_code() -->
<!-- ``` -->
...
<!-- ```{r simple-function-11-solution, exercise.reveal_solution = FALSE} -->
<!-- myfunc_3() -->
<!-- ``` -->

<!-- ```{r simple-function-11-check} -->
<!-- grade_this_code() -->
<!-- ``` -->
...
```{r vector-functions-2-solution, exercise.reveal_solution = FALSE}
rescale01 <- function(x){
  
}
```

```{r vector-functions-2-check}
grade_this_code()
```
...
```{r vector-functions-3-solution, exercise.reveal_solution = FALSE}
rescale01 <- function(x){
  (x - min(x, na.rm = TRUE)) / (max(x, na.rm = TRUE) - min(x, na.rm = TRUE))
}
```

```{r vector-functions-3-check}
grade_this_code()
```
### 
...
```{r vector-functions-4-solution, exercise.reveal_solution = FALSE}
rescale01(c(-10, 0, 10))
```

```{r vector-functions-4-check}
grade_this_code()
```
...
```{r vector-functions-5-solution, exercise.reveal_solution = FALSE}
df
```

```{r vector-functions-5-check}
grade_this_code()
```
...
```{r vector-functions-6-solution, exercise.reveal_solution = FALSE}
df |> 
  mutate(
    a = rescale01(a),
    b = rescale01(b),
...
```{r vector-functions-7-solution, exercise.reveal_solution = FALSE}
...
```{r vector-functions-8-solution, exercise.reveal_solution = FALSE}
first_upper <- function(x) {
  str_sub(x, 1, 1) <- str_to_upper(str_sub(x, 1, 1))
  x
}
...
```{r vector-functions-9-solution, exercise.reveal_solution = FALSE}
first_upper("hello")
```

```{r vector-functions-9-check}
grade_this_code()
```
...
```{r vector-functions-11-solution, exercise.reveal_solution = FALSE}
clean_number("$12,300")
clean_number("45%")
```

```{r vector-functions-11-check}
grade_this_code()
```
...
```{r vector-functions-12-solution, exercise.reveal_solution = FALSE}
commas <- function(x) {
  str_flatten(x, collapse = ", ", last = " and ")
}
```

```{r vector-functions-12-check}
grade_this_code()
```
...
```{r vector-functions-13-solution, exercise.reveal_solution = FALSE}
commas(c("cat","dog","piegon"))
```

```{r vector-functions-13-check}
grade_this_code()
```
...
```{r vector-functions-14-solution, exercise.reveal_solution = FALSE}
mape <- function(actual, predicted) {
  sum(abs((actual - predicted) / actual)) / length(actual)
}
```

```{r vector-functions-14-check}
grade_this_code()
```
...
```{r data-frame-functions-1-solution, exercise.reveal_solution = FALSE}
grouped_mean <- function(df, group_var, mean_var){
  
}
```

```{r data-frame-functions-1-check}
grade_this_code()
```
...
```{r data-frame-functions-2-solution, exercise.reveal_solution = FALSE}
# option 1 ----
grouped_mean <- function(df, group_var, mean_var){
  df |>
    group_by(group_var)|>
...
```{r data-frame-functions-2-check}
grade_this_code()
```
...
<!-- ```{r data-frame-functions-3-solution, exercise.reveal_solution = FALSE} -->
<!-- diamonds |>  -->
<!--   grouped_mean(cut, carat) -->
<!-- ``` -->

<!-- ```{r data-frame-functions-3-check} -->
<!-- grade_this_code() -->
<!-- ``` -->
...
To make this clear, consider the dataframe `df1`:
...
Start a pipe with `df1` to `grouped_mean()` and pass in `group` and `x`. While we aren't using it, we should note the data set has a column named `group_var`.
...
```{r data-frame-functions-4-solution, exercise.reveal_solution = FALSE}
df1 |> 
  grouped_mean(group, x)
```

```{r data-frame-functions-4-check}
grade_this_code()
```
...
Modify the code for `grouped_mean`. Rename the function to `grouped_mean1`. Modify the arguments within `group_by()` and `summarize()` to be enclosed with `{{}}`.
...
```{r data-frame-functions-5-solution, exercise.reveal_solution = FALSE}
grouped_mean1 <- function(df, group_var, mean_var) {
  df |> 
    group_by({{ group_var }}) |> 
    summarize(mean({{ mean_var }}))
...
```{r data-frame-functions-5-check}
grade_this_code()
```
...
```{r data-frame-functions-6-solution, exercise.reveal_solution = FALSE}
diamonds |> 
  grouped_mean1(cut, carat)
```

```{r data-frame-functions-6-check}
grade_this_code()
```
### 
...
So, when writing data frame functions, a key challenge is determining which arguments need to be embraced. Fortunately, this task is made easier because you can find the relevant information in the documentation 😄. In the documentation, there are two terms you should look for that correspond to the two most common sub-types of tidy evaluation:
...
Based on the code, it'll calculate the min, mean, median, max, count and null values. This makes it easier to get the shape of the data set.
...
```{r data-frame-functions-9-solution, exercise.reveal_solution = FALSE}
diamonds |> 
  summary6(carat)
```

```{r data-frame-functions-9-check}
grade_this_code()
```
...
```{r data-frame-functions-10-solution, exercise.reveal_solution = FALSE}
diamonds |> 
  group_by(cut) |>
  summary6(carat)
```

```{r data-frame-functions-10-check}
grade_this_code()
```
...
```{r data-frame-functions-11-solution, exercise.reveal_solution = FALSE}
count_prop <- function(df, var, sort = FALSE){
  
}
```

```{r data-frame-functions-11-check}
grade_this_code()
```
...
Copy the previous code, within the curly braces, start a pipe with df to `count()` and include `var` with `sort = sort`. Remember to enclose `var` in the body using `{{}}`. Then, extend the pipe to `mutate()` and pass in `prop = n/sum(n)`.
...
```{r data-frame-functions-12-solution, exercise.reveal_solution = FALSE}
count_prop <- function(df, var, sort = FALSE){
  df |>
    count({{ var }}, sort = sort) |>
    mutate(prop = n / sum(n))
...
```{r data-frame-functions-12-check}
grade_this_code()
```
...
```{r data-frame-functions-13-solution, exercise.reveal_solution = FALSE}
diamonds |>
  count_prop(clarity)
```

```{r data-frame-functions-13-check}
grade_this_code()
```
...
Other helper functions we could use is `filter()`, `arrange()` and `distinct()`. Let's make a function which finds distinct sorted values from a filtered data.
...
```{r data-frame-functions-14-solution, exercise.reveal_solution = FALSE}
unique_where <- function(df, condition, var) {
  df |> 
    filter({{ condition }})
...
```{r data-frame-functions-14-check}
grade_this_code()
```
...
```{r data-frame-functions-15, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r data-frame-functions-15-hint-1, eval = FALSE}
unique_where <- function(..., condition, ...) {
  df |> 
    filter({{ ... }})|>
    distinct({{...}})|>
...
```{r data-frame-functions-15-solution, exercise.reveal_solution = FALSE}
unique_where <- function(df, condition, var) {
  df |> 
    filter({{ condition }}) |>
    distinct({{var}}) |>
...
```{r data-frame-functions-15-check}
grade_this_code()
```
...
```{r data-frame-functions-16, exercise = TRUE}

```

```{r data-frame-functions-16-hint-1, eval = FALSE}
flights |> 
  unique_where(... == 12, dest)
```

```{r data-frame-functions-16-solution, exercise.reveal_solution = FALSE}
flights |> 
  unique_where(month == 12, dest)
```

```{r data-frame-functions-16-check}
grade_this_code()
```
...
The function first group the data by the vars, then it will summarize the missing values.

### Exercise 18

Start a pipe with `flights` to `count_missing()` and pass in `c(year, month, day), dep_time`.

```{r data-frame-functions-18, exercise = TRUE}

```

```{r data-frame-functions-18-hint-1, eval = FALSE}
flights |>
  count_missing(...,...)
```

<!-- ```{r data-frame-functions-18-solution, exercise.reveal_solution = FALSE} -->
<!-- flights |> -->
<!--   count_missing(c(year, month, day), dep_time) -->
<!-- ``` -->

<!-- ```{r data-frame-functions-18-check} -->
<!-- grade_this_code() -->
<!-- ``` -->
...
```{r data-frame-functions-19, exercise = TRUE}

```

```{r data-frame-functions-19-hint-1, eval = FALSE}
...
```{r data-frame-functions-19-solution, exercise.reveal_solution = FALSE}
count_missing1 <- function(df, group_vars, x_var) {
  df |> 
    group_by(pick({{ group_vars }})) |> 
    summarize(
...
```{r data-frame-functions-19-check}
grade_this_code()
```
...
```{r data-frame-functions-20, exercise = TRUE}

```

```{r data-frame-functions-20-hint-1, eval = FALSE}
flights |>
  ...(c(year, month, day), dep_time)
```

```{r data-frame-functions-20-solution, exercise.reveal_solution = FALSE}
flights |>
  count_missing1(c(year, month, day), dep_time)
```

```{r data-frame-functions-20-check}
grade_this_code()
```
...
```{r plot-functions-1-solution, exercise.reveal_solution = FALSE}
histogram <- function(df, var, binwidth = NULL){
  
}
```

```{r plot-functions-1-check}
grade_this_code()
```
...
```{r plot-functions-2-solution, exercise.reveal_solution = FALSE}
histogram <- function(df, var, binwidth = NULL){
  df |>
    ggplot(aes(x = {{x}}))+
    geom_histogram(binwidth = binwidth)
...
Start a pipe with `diamonds` data set to `histogram()` and set the first argument to `carat` and second to `0.1`.
...
```{r plot-functions-3-solution, exercise.reveal_solution = FALSE}
diamonds |> 
  histogram(carat, 0.1)
```

```{r plot-functions-3-check}
grade_this_code()
```

### 

To clarify and clear up any confusion, we already set `df` to `diamonds` with the pipe and setting then set the remaining values within the function call.
...
```{r plot-functions-4-solution, exercise.reveal_solution = FALSE}
diamonds |> 
  histogram(carat, 0.1) +
...
```{r plot-functions-5-solution, exercise.reveal_solution = FALSE}
linearity_check <- function(df ,x ,y){
  
}
```

```{r plot-functions-5-check}
grade_this_code()
```

### 

Next up let add code to the body.
...
```{r plot-functions-6-solution, exercise.reveal_solution = FALSE}
linearity_check <- function(df ,x ,y){
  df |>
    ggplot(aes(x = {{x}}, y = {{y}}))+
    geom_point()
...
```{r plot-functions-6-check}
grade_this_code()
```

### 

Let's now add a straight lie and a smooth line.
...
```{r plot-functions-7-solution, exercise.reveal_solution = FALSE}
linearity_check <- function(df, x, y) {
  df |>
    ggplot(aes(x = {{ x }}, y = {{ y }})) +
    geom_point() +
...
```{r plot-functions-8-solution, exercise.reveal_solution = FALSE}
linearity_check <- function(df, x, y) {
  df |>
    ggplot(aes(x = {{ x }}, y = {{ y }})) +
    geom_point() +
...
```{r plot-functions-9-solution, exercise.reveal_solution = FALSE}
starwars |>
  filter(mass < 1000)|>
  linearity_check(mass, height)
```

```{r plot-functions-9-check}
grade_this_code()
```
...
Maybe you want an alternative to colored scatter plots for very large data sets where overplotting is a problem, so hex plot would work out great. Below is code on how it works:
...
Now that we learn about using multiple variables, let's now learn how to set `labs()` using arguments in functions.
...
```{r plot-functions-11-solution, exercise.reveal_solution = FALSE}
sorted_bars <- function(df, var){
  
}
```

```{r plot-functions-11-check}
grade_this_code()
```
...
```{r plot-functions-12-solution, exercise.reveal_solution = FALSE}
sorted_bars <- function(df, var){
  df |> 
    mutate({{ var }} := fct_rev(fct_infreq({{ var }}))) 
}
...
```{r plot-functions-13-solution, exercise.reveal_solution = FALSE}
sorted_bars <- function(df, var){
  df |> 
    mutate({{ var }} := fct_rev(fct_infreq({{ var }}))) |>
    ggplot(aes(y = {{var}}))+
...
```{r plot-functions-13-check}
grade_this_code()
```
### 

You have now made a sorted bar graph function.

### Exercise 14

Let's now use the `filter()` function which is another helper function. Copy the previous code, change the name to `conditional_bars` , add another argument named `condition` before `var` in the argument list, and replace the `mutate()` with ` filter({{ condition }})`. Lastly, change the `y` to `x` so we will have vertical bars.

```{r plot-functions-14, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r plot-functions-14-hint-1, eval = FALSE}
...
```{r plot-functions-14-solution, exercise.reveal_solution = FALSE}
conditional_bars <- function(df, condition, var){
  df |> 
    filter({{ condition }}) |>
    ggplot(aes(x = {{var}}))+
...
```{r plot-functions-15, exercise = TRUE}

```

```{r plot-functions-15-hint-1, eval = FALSE}
diamonds |>
  ...(cut == "Good", ...)
```

```{r plot-functions-15-solution, exercise.reveal_solution = FALSE}
diamonds |> 
  conditional_bars(cut == "Good", clarity)
```

```{r plot-functions-15-check}
grade_this_code()
```
...
Modify the `histogram1` as follows: before the start of the pipe, create a new variable `label` and assign it the value of `rlang::englue("A histogram of {{ var }} with binwidth { binwidth }")`. Then, within the `df` pipe, add the `labs()` layer after `geom_histogram()`, and set `title = label` within `labs()`.

```{r plot-functions-17, exercise = TRUE}
histogram1 <- function(df, var, binwidth) {
...
```{r plot-functions-17-hint-1, eval = FALSE}
...
```{r plot-functions-17-solution, exercise.reveal_solution = FALSE}
histogram1 <- function(df, var, binwidth) {
  label = rlang::englue("A histogram of {{ var }} with binwidth { binwidth }")
...
Now let's use the function, so start a pipe with `diamonds` to `histogram1()` and pass in `carat` and `0.1` as the arguments.

```{r plot-functions-18, exercise = TRUE}

```

```{r plot-functions-18-hint-1, eval = FALSE}
diamonds |>
  ...(carat, ...)
```

```{r plot-functions-18-solution, exercise.reveal_solution = FALSE}
diamonds |>
  histogram1(carat, .1)
```

```{r plot-functions-18-check}
grade_this_code()
```
...
Let's make the x and y axis look better so copy the code and after `histogram1()` add the `labs()` layer and set `x` to `"Size (in carats)"` and `y` to `"Number of diamonds"`.

```{r plot-functions-19, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r plot-functions-19-hint-1, eval = FALSE}
... |>
  ...(carat, ...) +
  labs(x = ..., y = ...)
```

```{r plot-functions-19-solution, exercise.reveal_solution = FALSE}
diamonds |>
  histogram1(carat, .1) +
  labs(x = "Size (in carats)",
       y = "Number of diamonds")
